...
`SectionStaticHeader` (exported beside `Section`) is that `empty` header on its own — title, icon, `actions`, 32px, no toggle. It exists for a block that is always open by construction rather than by state: the Properties panel's Module block (`ModuleBlock.tsx`) draws its title with it, so the selected element's own header is the same recipe as every section title under it.

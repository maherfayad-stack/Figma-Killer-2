...
| AI chat snapshot (optional HTTP body field) | `safeParseValue(StudioAgentSnapshotSchema, snapshot)` — falls back to an empty placeholder on failure; never crashes the stream | `server/ai/handlers/chat.ts` |

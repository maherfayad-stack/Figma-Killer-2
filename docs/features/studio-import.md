...
Because `text`/`imports` are strings the client sends, nothing about them is trusted merely because they look plausible: both are parsed in a throwaway in-memory `Project` and refused (`invalid-import`, `invalid-source`) if an import doesn't parse as exactly one `ImportDeclaration`, or if the spliced candidate carries MORE syntactic diagnostics than the original file did. Both refusals leave the file untouched.

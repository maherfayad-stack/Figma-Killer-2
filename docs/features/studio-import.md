...
- **Computed `className` whose interpolation isn't statically resolvable.** `` className={`esb esb--${tone}`} `` with an unresolvable `tone` is dropped entirely on an ORDINARY element — `extractProps`' evaluator fallback only accepts a fully-resolved `StaticValue` (`tryResolveExpression` checks `result.kind === 'literal'`), so the `unresolved` variant's own `partial` prefix (`evaluateTemplate`'s `unresolved(reason, partial)`, Tier A) is computed but never read back out: no `className` reaches the node at all, not even a truncated one. **The static-prefix fallback IS real, but only inside `componentSubstitution.ts`'s call-site `className` re-read during local-component inlining** (see [above](#a-components-own-jsx-is-re-read-against-the-call-sites-values)) — a narrower, different code path than this bullet used to imply. As of WS-2.2 the fully-resolvable case is much narrower than it used to be — a CSS Modules value (`styles.card`), a resolved const, or a `cn()`/`clsx()` call now resolves through the evaluator — but a genuinely runtime-only interpolation (component state, an unresolvable prop) on a non-inlined element now renders with no class at all. The plan's WS-2.4 variant probe (pick the default variant from an enumerable prop union, record a `resolution.note`) is not implemented.

...
- CSS arrives in each iframe via five injectors: `EditorChromeInjector` (unlayered), `ProjectCssInjector` (`@layer vendor` — package CSS, read-only), `AuthoredCssInjector` (`@layer user-authored` — the project's OWN `.css`, raw and unparsed), `ClassStyleInjector` (`@layer reset` for the publisher reset, **CMS pages only** — see below, + `@layer user-authored` for the class registry), `UserStylesheetInjector` (`@layer user-authored`). The order is `reset` → `vendor` → `user-authored`: the user's own edits always win over a package default, and the zero-specificity reset loses to both (above `vendor` it annihilated every design-system component) — see "CSS injection into iframes" below.
- Wheel, pointer, and keyboard events are forwarded from inside the iframe to the parent's gesture / reorder-drag / shortcut handlers. `Tab` is cancelled inside the frame (no tab-walking the design preview) and forwarded, where it selects the next sibling.
...
IframeFrameSurface
  <iframe srcDoc="<!doctype html><html>…">
    ...
    ├── AuthoredCssInjector        (head: @layer user-authored — project's own CSS, raw)
    ...
    ├── ClassStyleInjector     (head: @layer reset — publisher reset, CMS pages only; @layer user-authored — class registry)
...
|---|---|---|---|---|---|---|---|
| Design canvas frame | `'canvas'` (default) | grows to content | none (frame scrolls with canvas pan) | forwarded to parent pan/zoom (`Shift+wheel` pans sideways) | forwarded for space-left-drag pan | forwarded to parent `document` (shortcuts); `Tab` cancelled in-frame, then forwarded | applied (cursor, user-select, outline overrides) |
...
Nine `<style>` elements are injected per iframe (three from `ClassStyleInjector`, one each from the other six injectors), plus two design-frame-only injectors that inject none in live mode:
...
| `ProjectCssInjector` | `mc-vendor` | `@layer vendor` | **Read-only** vendor CSS (WS-2.3): the built-in design system's bundled stylesheet (Studio's own vendored `vendor/alm-design-system/dist/index.css`, `?inline`-imported at Studio's build time) concatenated with the OPEN project's own bare-specifier package CSS (`import '@acme/ui/dist/style.css'`, resolved against the project's own `node_modules` server-side by `styleCompile.ts`'s `collectVendorCss`, threaded onto the client via `GET /admin/api/studio/load`'s `vendorCss` field). Opens with a bare `@layer vendor, user-authored;` pre-declaration — see "Vendor vs. user-authored ordering" below. Never parsed into a `StyleRule`; never editable. |
| `AuthoredCssInjector` | `mc-authored` | `@layer user-authored` | The project's own `.css` files plus WS-2.1's compiled Tailwind/Sass/PostCSS and CSS-Modules-renamed output, injected **raw and unparsed** — the same treatment `mc-vendor` gets, and for the same reason. `cssToStyleRules` parses through happy-dom's CSSOM, which SILENTLY DROPS declarations it cannot handle (`color-mix()`, `Canvas`/`CanvasText`, slash-alpha `rgb(0 0 0 / .2)`), so a registry-regenerated sheet is not a faithful copy of what the author wrote. This is the renderer; `mc-classes` is now only an OVERLAY (see below). Always `insertBefore(head.firstChild)` so it precedes `mc-classes` in source order regardless of React mount timing. |
| `CanvasAnimationInjector` | `studio-canvas-animation` | **unlayered**, `!important` | Design frames only — see "Freeze and unroll" below. Its rules also carry the inspector's scrub position and play-once phase (W5-5). |
...
| `ClassStyleInjector` | `mc-classes` | `@layer reset` (CMS pages only) + `@layer user-authored` | Publisher reset (`@layer reset`, gated off in Studio mode — see below) + framework CSS + class registry CSS (`@layer user-authored`). Since `mc-authored` became the renderer, the class registry half emits **only** editor-authored rules and imported rules edited this session (`styleRuleNeedsCanvasOverlay`) — raw is "what is on disk", this is "what has changed and is not yet written back". Emitting the whole registry here would restate every imported rule from the lossy parse and undo `mc-authored`. Also opens with the `@layer vendor, user-authored;` pre-declaration. |
...
`PUBLISHER_RESET_CSS` (`src/core/publisher/reset.ts`) is the correct baseline for a **CMS-authored** page: one built from the module engine, which genuinely ships no stylesheet of its own, so without a reset it would fall back to raw UA defaults the CMS publisher never intends the page to show. `render.ts` still injects it unconditionally into the real published HTML for that reason — untouched by this section.
...
Studio's own built-in one does exactly this — which means an UNSET attribute
...
**`CanvasAnimationInjector`** (`src/admin/pages/site/canvas/CanvasAnimationInjector.tsx`) freezes every source of motion: CSS animations run once and hold (`animation-iteration-count: 1; animation-fill-mode: forwards`, or `animation-play-state: paused` when the `freezePoint` prop is `'start'` instead of the default `'end'`, or a negative `animation-delay` against a normalised `animation-duration: 1s` when `freezePoint` is a 0…1 fraction — W5-5; `'start'` and `'end'` are the endpoints of that same axis, and the inspector's scrub moves along it board-wide through `animationScrubStore.ts`. See the file's docblock for when each is correct), CSS transitions are killed (`transition: none`), smooth scrolling is disabled (`scroll-behavior: auto`), `<video>`/`<audio>` are paused and stripped of `autoplay` (both at mount and for elements inserted later, via a `MutationObserver`), and `window.matchMedia` inside the iframe is patched so a `(prefers-reduced-motion: reduce)` JS check reports true. **What it cannot freeze:** animated GIF/WebP/APNG frame-advance (decoded by the image codec, not CSS — no rule can pause it), JS-driven animation (framer-motion, GSAP, a raw `requestAnimationFrame` loop — only runs when "Run scripts" is on, and this injector does not intercept `requestAnimationFrame`), `<canvas>`/WebGL loops, and the browser's own native CSS `@media (prefers-reduced-motion: reduce)` evaluation (an OS-level signal; only `matchMedia`, a JS API, can be patched from inside the iframe — a stylesheet `@media` block written directly by the author is unaffected).
...
**A gesture that can be cancelled listens on BOTH documents.** Escape during an element drag (`useCanvasReorderDrag`) and during an element resize (`useElementResizeDrag`) is a native `keydown` handler attached to the parent `window` *and* to the frame's own `contentDocument`, because the cursor may be over either when the key is pressed and a keystroke raised inside an iframe never reaches the parent window. This is deliberately NOT routed through the keyboard clone bridge above: the clone exists to reach the editor's global shortcut listeners, while a cancel must be handled by the session that owns the pointer and by nothing else.
...
- Source-of-truth files:
  ...
  - `src/admin/pages/site/canvas/animationScrubStore.ts` — the board-wide scrub position + play-once phase the injector reads (W5-5)

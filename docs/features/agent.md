...
The full argv, widened across WS-11 steps 2–3 and sec-XX:

```
claude -p <prompt> --output-format stream-json --verbose --model <id>
  --effort medium --permission-mode default --tools <'Task' | 'Task,Read' | 'Read' | ''>
...
**Native tool surface (sec-XX).** Before this fix, the spawned session carried NO `--tools`/`--allowedTools` restriction at all — the top-level `claude` process could reach its native `Bash`/`Write`/`Edit`/`Read`/`Glob`/`Grep`/`WebFetch` tools directly, bypassing every containment check, trust-tier gate, and `.studio/` exclusion the MCP tools (`server/ai/mcp/tools/studio/*`) enforce. `resolveNativeToolAllowlist` (`claudeCli.ts`) now grants at most two built-ins, computed fresh per turn, never omitted: `Task` only when a real project is open (subagent dispatch — the one thing genuinely not reachable through MCP, see "Subagent roster" below) and `Read` only when this turn staged an attachment (the CLI's own built-in tool is how it views a staged image/file — see `claudeCliAttachments.ts`). Everything else is withheld unconditionally, on every turn, at every trust tier — trust tiers gate MCP-mediated capabilities like `studio_install_deps`, never a raw shell. `--tools` is a hard availability list, independent of and prior to `--permission-mode`, so it holds even under a user-selected `bypassPermissions`. **Not independently proven against the real binary with a live turn** whether the CLI's own built-in default subagent types (`general-purpose`, confirmed via the installed binary's embedded prompt text to normally carry "all tools including file editing, writing, and bash") are bounded by this same `--tools` ceiling when dispatched via `Task` — strong circumstantial evidence only (the CLI's own dynamically-built per-session agent/tool listing, and `agentRoster.ts`'s pre-existing `assertKnownTools` design assumption that the top-level session's toolset is already the ceiling for every subagent). See `STATE.md`'s `sec-XX` entry for the residual-risk note.

**Workspace `cwd` (step 2).** A real chat turn spawns in the resolved, containment-checked project directory — `resolveClaudeCliWorkspaceCwd(req.workspaceDir, projectsRoot)` (`claudeCliEnv.ts`), fed from `useAdminUi`'s open `studioProject.dir` through `agentSlice.ts` → `AiChatRequestBody.workspaceDir` → `AiStreamRequest.workspaceDir`. This is what makes `.claude/agents/*.md` auto-discovery, `CLAUDE.md` discovery, and the tools' own view of the project work at all — spawn in the wrong place and WS-12's entire subagent roster silently doesn't exist. Containment mirrors `appRoot.ts`: both the requested path and the projects root are resolved through `realpathSync` before the prefix check, so a symlink pulled in from a GitHub import can't escape it. No workspace open, or containment fails → falls back to the per-user `CLAUDE_CONFIG_DIR` (a documented degraded case, not a crash). The availability **probe** always stays in the config dir — it must never risk a real project's `CLAUDE.md` cache-creation cost (see the cost warning below).
...
**Deliberately withheld from every tool and every agent, no exceptions:** a shell tool, a raw file-write tool, and trust promotion. An agent may *ask* the user to promote a project's trust tier; it may never perform the promotion itself. Until sec-XX, this was true of every MCP tool and every generated subagent but NOT of the top-level `claude` process itself, which held its full native built-in toolset with no restriction — see "Native tool surface (sec-XX)" above for the fix that closed that gap.
...
`generateStudioAgentRoster(dir)` is called from `claudeCli.ts`'s `streamClaudeCli`, right before every real turn spawns (`workspaceDir` validated non-null) — "written beside the MCP config", alongside `buildMcpConfig`. Best-effort: a probe failure logs and degrades the turn to "no subagents", never blocks the chat (`claudeCli.test.ts`'s "a roster-generation failure degrades the turn, never aborts it").

**perf-06 — the warm path is gated, not rebuilt every turn.** The function used to rebuild all ten agents' worth of markdown plus 7 reference files and hash-compare each of the 17 targets on EVERY call — measured ~19ms warm against a real 46-file design-system corpus, paid on every turn after the first for a project the user keeps chatting about. `server/handlers/studio/agentRosterManifest.ts` now gates the whole rebuild behind two cheap, independent checks: `computeRosterFingerprint` (a hash of the resolved `ProjectProfile`, the design-system CSS stat key, the ALM package docs' stat witness, and a roster-definition version — a match proves the output would be byte-identical to last time) AND `allOwnedFilesUnchangedSince` (a `statSync` per target file against the size/mtime recorded at the last write — the clobber-protection half, since the fingerprint only covers INPUTS and cannot see a hand-edited OUTPUT file on its own). Both must hold for the fast path; either one failing forces the full rebuild, which is what still lets the existing hash-comparison loop detect and report a hand edit. Measured: ~19ms → ~2-3ms warm (`bun run bench:agent-turn`). Separately, `resolveProjectProfile` itself re-probes from scratch on every call for a project with no persisted profile in `.studio/meta.json` (true for anything imported without a `package.json`, e.g. the design-token wizard) — `resolveProjectProfilePersisting` (`projectProbe.ts`) heals that the first time the roster generator runs, persisting the freshly-probed profile so every later call (from this generator, `studio_project_profile`, or the CMS-prompt path in `chat.ts`) hits the cached branch instead (~8ms → ~0.1ms).

**The ten-agent roster, plus an eleventh generated only for an approved Figma-capable project:**

| Agent | Tools | Owns |
|---|---|---|
| `screen-scout` | 6 read tools | Read-only orientation — file:line answers, never opinions. Holds no write tool at all. |
| `screen-builder` | create/read/find/list-components/find-component/apply-edits | Scaffolds and composes in Canonical JSX; checks the design-system catalog **before** hand-rolling markup; owns the insert/move/delete batch and the staleness discipline. |
| `style-surgeon` | apply-edits + read + list-components/find-component | Styling only, through the project's existing mechanism — never a second one; checks for an existing component/token before writing a rule. |
...
| `almosafer-ds-expert` | project-profile/find/list-components/find-component/read-package-doc/read | ALM 2.0 authority — see "The ALM design-system expert" below. |
...
| `figma-asset-scout` (project-conditional) | read/list-component-bindings/find-component/fetch-remote-asset + two `mcp__<figma-server>__*` grants | Locates a Figma node for a component, pulls its export, lands it via `studio_fetch_remote_asset` — never composes it into a screen. See "Figma asset workflow" below. |
...
**Design-system-first is prompt + tool-ordering guidance, not a mechanical gate.** `studio_apply_edits` never refuses a hand-rolled element — there is no block. Instead `screen-builder`, `style-surgeon`, and `almosafer-ds-expert` all hold `studio_list_components`/`studio_find_component` (mcp-08's headless catalog reads, §"Site catalog read tools" below) and their prompts state one priority order: an existing component, then a design token, then hand-written CSS — and `screen-builder`'s prompt requires calling `studio_list_components` **before** composing anything, not only when something looks missing. Every one of those three prompts also states the honesty constraint this closes: `studio_list_components` reads a package's own `.d.ts`/`.tsx` type declarations, so a design system shipping bundled, untyped JSX (the real `@alm-design/design-system` install) returns **zero** components even though it is real — the prompt tells the agent to check the response's `designSystems`/`note` fields rather than concluding "no design system" from an empty array, and to fall back to `design-system.md`'s own BEM class index, the design system's own MCP server if the project has approved one, or a sibling screen's existing classes. `studio-design-principles.md` (§7.4, below) states the same order and the same honesty constraint as the general house rule `design-critic` reviews against.

**The ALM design-system expert.** `@alm-design/design-system` ships its own `CLAUDE.md` (technical API) and `design.md` (intent/content guidelines) inside `node_modules`. §7.2's rule is "read from the installed package, never vendor a copy" — but a subagent literally cannot reach `node_modules` itself: `studio_read_file`, the file-read tool for the user's OWN source, refuses any `node_modules` segment by design (the same containment guard every other Studio read uses). `agentRoster.ts` used to read both files **server-side, at roster-generation time**, capped at 50 KB each and embedded whole — the real package's own files are ~103 KB/106 KB, so that embed silently came up empty even for a fully, correctly installed project, and `almosafer-ds-expert` fell into its "nothing to consult" branch despite the package being right there. Fixed by embedding an **OUTLINE** instead (`agentRosterDocOutline.ts`'s `buildDocOutline` — every heading with its body's byte size, mirroring what `studio_read_package_doc`'s own `outline: true` response does) rather than raising the cap: 100+ KB of prose regenerated into a subagent prompt on every real chat turn is its own cost, independent of whether it happens to fit under some ceiling. The agent pulls the one section it actually needs at call time via `studio_read_package_doc({ package, doc, section })`. This generator cannot call that tool itself — it must stay synchronous (`claudeCli.ts` calls it un-awaited, on the real turn's critical path, right before the subprocess spawns), while every `AiTool` handler is `async` — so `agentRosterDocOutline.ts` is a small, independent copy of the same heading-split logic, not a shared import. When neither file embeds (no dependency, or files missing from `node_modules`) but a design system was still detected (see `design-system.md` below), the agent is told to read that instead of a package README. Only when NO design system exists at all does it degrade to "I have nothing authoritative to consult" rather than reasoning about ALM components from memory.

**Reference files (§7.4)** — `{canonical-jsx,studio-invariants,node-ids-and-writeback,studio-tools,studio-design-principles,project-conventions}.md`, plus `design-system.md` when a design system was detected — written under `<project>/.claude/`, and every subagent prompt names the exact project-relative path (`.claude/studio-invariants.md`) rather than the old "in this same `.claude/` directory" wording. That wording was not just imprecise, it was **wrong**: `buildReferenceFiles` wrote each file's `relPath` bare (`'canonical-jsx.md'`), which lands at the project ROOT, not `.claude/` — an agent that followed its own prompt literally got a not-found. Fixed by moving every reference file's write target under `.claude/` (`referencePath()` in `agentRoster.ts`, the single source of truth both the write target and every prompt's pointer text use) and naming the exact path in every prompt instead of gesturing at proximity. `.claude/` is not in `EXCLUDED_WORKSPACE_DIR_NAMES` (`src/core/page-parser/workspaceFiles.ts`), so `studio_read_file`'s containment check reaches it exactly as it reached the project root. A project that regenerated before this fix keeps its stale root-level copies on disk, un-deleted — the manifest (keyed by `relPath`) simply stops tracking the old bare-name keys on the next regeneration, so they go permanently unmanaged rather than being bulk-deleted by filename, which would be the riskier operation. Each reference file is a compact **pointer**, not a restatement — `canonical-jsx.md`'s generated file explicitly names `docs/reference/canonical-jsx.md` as the full contract rather than duplicating it (a restated copy would drift the first time the real doc changes). `studio-tools.md` is **generated from the live tool registry**, never hand-written — a hand-written tool list is wrong the first time a tool is renamed, and every agent would inherit the error at once. `project-conventions.md` is the one genuinely per-project file — framework, pages directory, styling mechanism, component packages, straight from `studio_project_profile`.

**`design-system.md` (WS-? — design-system-digest)** — a MAP of a project's own design-system CSS, not the CSS itself, built by `server/handlers/studio/designSystemDigest.ts` from the SAME engine `tokenExtract.ts`'s `project-css`/`vendor-css` sources already use (`tokenExtractCssScan.ts`'s `collectRootScopeMaps`/`classifyDeclaration` — no new CSS parser). It exists because every OTHER mechanism above is keyed on `profile.componentPackages`, which is itself keyed on `node_modules` — a project whose design system arrived through the "Import design tokens" wizard's CSS-only copy (`styles/imported/<slug>/`, `designImport.ts`) has no `package.json` at all, so `componentPackages` is permanently empty and both the CLAUDE.md/design.md embed above and `studio_read_package_doc` have nothing to work with, even though the tokens and component stylesheets are sitting right there on disk. Before this file existed, an agent working against such a project re-read the whole design system from raw CSS on every single turn — for the real ALM corpus, 46 files / 171 KB cold, every turn, before any real work started.

`server/handlers/studio/designSystemDetect.ts`'s `detectDesignSystems` feeds `ProjectProfile.designSystems`: one entry per `componentPackages` name (`source: 'node-modules'`, root inside `node_modules/`) AND one per immediate `styles/imported/<slug>/` subdirectory (`source: 'imported'`, root always relative to the PROJECT directory, never the app root — the import wizard writes there unconditionally). The digest itself covers all five token families — colors, typography, spacing, **radius, elevation** (the last two `FrameworkSettings` has no home for at all: `--rounded-*` doesn't match `classifyDeclaration`'s own spacing name hint, and a shadow's multi-part value never passes the color check) — plus a one-line-per-component index (BEM block class name, every modifier variant found, the file to open for the full rule). Cached under `.studio/cache/design-system-<hash>.md`, the hash covering every discovered `.css` file's path/size/mtime, the same convention `styleCompile.ts` uses for `styles-<hash>.{css,json}`; `.studio/cache/` is gitignored, so this is disposable, regenerable output. Works whether or not `.studio/framework.json` (`tokenExtract.ts`'s own persisted output, a different artefact) exists at all — the digest reads CSS directly, every time.

**Regeneration never clobbers a user edit (trap #12).** `.claude/.studio-generated.json` (owned by `agentRosterManifest.ts`) records the hash, size, and mtime of what Studio itself last wrote for each generated file, plus the regeneration fingerprint (perf-06, above). A file is only overwritten when its on-disk content still matches that recorded hash — i.e. the user hasn't touched it since. A file that doesn't match (hand-edited, or never Studio-written) is left alone and reported `skipped`, never silently clobbered.

### A subagent can hold a vetted MCP-namespaced tool (mcp-tooling)

Tool-name vetting used to check ONLY `studioAgentTools` — Studio's own native registry — so naming ANY project-approved external MCP server's tool (a Figma MCP server, most concretely) in a roster definition threw inside `generateStudioAgentRoster`'s own try/catch, silently degrading the WHOLE turn to zero subagents. Not "that one agent is unavailable" — every subagent, because the failure is caught above the per-agent level.

`server/handlers/studio/agentRosterMcpTools.ts`'s `assertKnownAgentTools` is the fix — a tool name is now known when it is EITHER a real `studioAgentTools` entry, OR shaped `mcp__<server>__<tool>` where `<server>` is a name THIS PROJECT has actually approved (`resolveApprovedMcpServerNames` reads exactly the same two lists — `.mcp.json` names in `.studio/meta.json`'s `approvedMcpServers`, and Studio-registered servers in `approvedRegisteredMcpServers` — `claudeCli.ts` itself reads to decide what merges into `--mcp-config`). **What "vetted" means:** tied to real per-project approval, checked by literal prefix match against each actually-approved server name — never a generic `mcp__*` regex, which would let a roster entry name a tool from a server the project never approved. This is checked at SERVER granularity, not per-tool, deliberately: the main agent's own `--mcp-config` merge is ALSO scoped at server granularity (once "figma" is approved, the main agent can already call every tool that server exposes — `resolveNativeToolAllowlist`'s `--tools` ceiling only ever bounds the CLI's native built-ins, never an MCP-sourced tool name), so a subagent naming one or two specific tools from an approved server is always a STRICT SUBSET of what the main agent can already reach — "hold no tool the main agent itself does not have" still holds. An entry naming `mcp__<unapproved>__anything` still throws, exactly like an unknown native name always has. `mcpServerFingerprintWitness` folds every approved server's name/approved/summary into `computeRosterFingerprint` (never a secret value), so approving or revoking a server forces a full roster regeneration instead of silently going stale behind the fast (unchanged-fingerprint) path.

### Figma asset workflow — `figma-asset-scout` and `studio_fetch_remote_asset`

`server/handlers/studio/agentRosterFigma.ts` — a heuristic (`findApprovedFigmaServer`, name/summary containing "figma", case-insensitive — the same order of confidence `designSystemDetect.ts` already uses for "does this folder look like a design system") detects an approved Figma-capable MCP server and, only then, generates two things: **`.claude/figma.md`** (same conditional pattern as `design-system.md`) and the **`figma-asset-scout`** subagent.

`figma.md` teaches the real workflow (mcp-09's own findings), not generic MCP advice:
1. **Resolve the fileKey/nodeId PER COMPONENT** via `studio_list_component_bindings` — never assume one Figma file for a whole project. Confirmed against this repo's own ALM design system: 27 components map into one Figma file, `Checkbox`/`Radio` map into a completely separate one.
2. **Check `nodeIdPlaceholder` before fetching anything** — 5 of 29 real bindings in that same corpus are unfilled `figma connect create` scaffolds (`node-id=REPLACE-ME`), not resolvable references.
3. **Code Connect is the cheap path** — a component with a real binding already carries its own Figma node URL; only search Figma from scratch for one with no binding at all.
4. **Land via `studio_fetch_remote_asset`, not `studio_upload_asset`** — the whole point: bytes never transit the model.

`figma-asset-scout` is deliberately narrow (`studio_read_file` for its own `figma.md` reference, `studio_list_component_bindings`, `studio_find_component`, `studio_fetch_remote_asset`, plus `mcp__<server>__get_metadata`/`mcp__<server>__get_image`) — no `studio_apply_edits`, no structural tool. It locates, pulls, and lands; composing the result into a screen is a composing agent's (`screen-builder`'s) job. **The two `mcp__<server>__*` tool names are an honest, unverified assumption** — Figma's officially documented Dev Mode MCP Server's tool names, the most common "a Figma MCP" a user means today, picked because this generator has no live connection to the approved server at roster-generation time (synchronous, on the critical path before every real chat turn spawns) and so cannot ask it what tools it actually exposes. The subagent's own prompt says this explicitly and tells it to check its own available tools before assuming these are present. This was **not dogfooded against a live Figma MCP connection** — correcting the two literals once one is connected and probed is a roster-content change, not an architecture change.

**`studio_fetch_remote_asset`** (`server/ai/mcp/tools/studio/remoteAssetTools.ts`, `execution: 'server'`, `studio.write`) closes the "bytes never transit the model" gap `studio_upload_asset`'s `imageBase64` input can't: an agent that gets an image URL back from an approved Figma MCP tool hands the URL here instead of round-tripping the asset's bytes through its own context (base64 in, base64 out — real money for anything but a tiny icon). The fetch is untrusted-URL-safe: scheme restricted to `http:`/`https:`, no redirect ever followed (`redirect: 'error'` — the strict form of "never follow a redirect to an arbitrary host"), the response capped at 25 MB by STREAMED byte count (`readBytesWithLimit`, the same primitive the GitHub zipball fetch uses), and the resulting bytes land through `landAssetBytes` (`server/handlers/studio/assetLanding.ts`) — the SAME magic-number sniff, SVG sanitization, and containment-checked write `studio_upload_asset`'s browser upload already uses, extracted so the two callers can never drift. **A real, closed gap found along the way:** `assetUpload.ts` sniffed SVG content and wrote it verbatim, with no sanitization step at all — `server/handlers/cms/mediaUpload.ts` (the CMS media-library path) already ran every SVG through `sanitizeSvgBytes` before landing it; `assetUpload.ts` never did. Fixed once, in the shared `landAssetBytes`, so both callers get it. **Not hardened against SSRF via DNS rebinding or a private/internal IP literal in the URL** — flagged for security-guard review before this tool is trusted with a URL from a source less predictable than an approved MCP tool's own response.

**Gates (§9):**
1. Every tool named in any agent definition is either a real `studioAgentTools` entry or a vetted `mcp__<approved-server>__<tool>` name for that project — `agentRoster.test.ts` (base ten) + `agentRosterMcpTools.test.ts`/`agentRosterFigma.test.ts` (the vetting mechanism and `figma-asset-scout` itself), mirrors `fidelityCodes.test.ts`'s registry⇄doc pattern (extended from the prompt-only version `systemPrompt.test.ts` already gates).
2. No generated subagent holds a tool the main agent lacks — every native roster tool is sourced from `studioAgentTools` itself, and every `mcp__` grant is bounded to a server the project has approved (the same servers the main agent's own `--mcp-config` merges in) — asserted directly rather than assumed.
...
**Attachments — images AND files route to the CLI by staged file path** (`claudeCliDriver`'s `visionInput` capability flipped to `true`). There is no confirmed `-p` mechanism for inline image bytes (`--input-format stream-json`'s stdin shape remains unverified, per WS-11's own finding), so `claudeCliAttachments.ts` writes each attachment to a fresh, turn-scoped temp directory and appends its absolute path to the prompt text — the CLI's own built-in `Read` tool does the actual reading. Since sec-XX, `Read` is the ONE native built-in the session's `--tools` allowlist grants, and only on a turn that actually staged something here (see "Native tool surface (sec-XX)" above) — before that fix the top-level session held its full native built-in toolset unconditionally, `Read` included but not scoped to this use. The staging directory is torn down unconditionally in the driver's own `finally` block, alongside MCP connector revocation.

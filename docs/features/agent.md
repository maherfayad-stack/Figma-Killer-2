...
| `studio_fetch_remote_asset` | `server` | `studio.write` | `write` | Fetch a URL server-side and land it as a project image; bytes never transit the model |
...
- Source-of-truth files:
  ...
  - `server/ai/drivers/http/toolLoop.ts` — provider loop, its two ceilings (`MAX_TOOL_ROUNDS` and the per-turn write ledger behind `duplicate-call`, both in `toolLoopBounds.ts`), grouped (concurrent-observer) tool dispatch keyed on `sideEffects`, the `input-schema-mismatch` refusal (`toolInputRefusal.ts`, via `execTool.ts`), heavy tool-result elision as a per-request projection, message cache-breakpoint policy, and one provider-triggered historical-image fallback

...
The full argv, widened across WS-11 steps 2–3:

```
claude -p <prompt> --output-format stream-json --verbose --model <id>
  --effort medium --permission-mode default
...
This is also the supported route for a project's Figma MCP server. **The approval UI is not built yet** — approving currently means adding the server's name to `.studio/meta.json` by hand. `listProjectMcpServers` already returns each server with a human-readable `summary` (its command line or URL) for that prompt to render, so consent can be informed rather than a yes/no on a bare name.

...
| `.claude/design-system-icons.md` | Every icon the package ships: the `*Icon` components importable by name, and each `src/icons/<catalog>/` SVG catalog with the exact import to write (`renderIconReference`). |
...
| `studio_import_figma_frame` | server | `studio.write` | The whole "a Figma link arrived" ritual in ONE call: register the export as a strict spec reference, ingest the variables, size the board frame to the frame's own `absoluteBoundingBox` |
| `studio_register_design_reference` | server | `studio.write` | Store a design's ORIGINAL bytes under `.studio/references/`, scoped to a page |
| `studio_list_design_references` | server | read | What is armed for this project/page, metadata only |
| `studio_read_design_reference` | server | read | One reference's metadata, optionally its image bytes |
| `studio_ingest_design_variables` | server | `studio.write` | Store the design tool's OWN variable table, as handed over by the agent |
| `studio_list_design_variables` | server | read | Ingested variable sets, as summaries |
| `studio_read_design_variable_set` | server | read | One set's actual name/value entries, with Studio's normalisation shown |
| `studio_set_frames` | server | `studio.write` | Bulk-resize board frames in `.studio/boards.json` |
| `studio_set_frame_axes` | browser (live board) | `studio.write` | Flip one frame's preview direction / colour scheme / locale |
| `studio_duplicate_frame_as_variant` | browser (live board) | `studio.write` | The same page twice on the board, under different axes |
| `studio_list_comments` | server | read | The user's board pins as a work queue, each with an anchor-confidence verdict |
| `studio_reply_comment` | server | `studio.write` | Post an AI-tagged reply into a thread |
| `studio_resolve_comment` | server | `studio.write` | Resolve/reopen a thread; refuses on a drifted or detached anchor |
| `studio_project_profile` | server | read | "What am I working with" in one call — framework, styling, packages, dark mode, warnings |
| `studio_list_pages` | server | read | Every page/board frame: id, title, slug, node count |
...
| `studio_install_status` | server | read | Poll that job: `running`/`done`/`failed`/`timeout`, plus log and exit code |

**Registry-only Studio tools** — in `mcpToolsForCapabilities` for an *unbound* external MCP client (Claude Code in a terminal, a remote agent), never offered to the in-canvas agent. The exclusions are justified one by one in `agentToolNames.ts`; the short version is that everything filesystem-shaped is strictly slower than the native `Read`/`Write`/`Edit`/`Glob`/`Grep` the driver already grants, and the two measurement tools take base64 bytes the in-canvas agent cannot produce from an MCP image block.

| Tool | Gate | One line |
|---|---|---|
| `studio_list_projects` | read | Every project under `studio-workspace/`, with page count and probed profile |
| `studio_list_files` | read | The project's real file tree as relative POSIX paths; generated/dependency folders never listed |
| `studio_read_file` | read | A whole workspace file by relative path, plus the WS-13 canonical-JSX verdict for `.tsx`/`.jsx` |
| `studio_find_nodes` | read | Query nodes by moduleId/tag/class/text/lock state — "show me everything that failed to resolve" |
| `studio_get_node_source` | read | Decode a node id to `{ file, line, col, snippet }`. Refuses for synthetic and `.map`-iteration ids |
...
**Not done (W9-3):** the creative and balanced halves of the mode-aware Stop gate. Creative's DONE is a passing `quality_check` since the last write, which needs a quality-check verification record that does not exist yet; balanced's "every deviation named" is not machine-checkable from the gate's side. Both currently fall through to the pre-W9-2 rule (any post-write passing compare), which is the safe direction — it asks for a measurement, it never waves a page through.
...
**`studio_import_figma_frame({ dir?, pageId, url?, exportPath?, node?, variables?, mode?, label? })`** is that ritual as one call, `execution: 'server'`, `mutates: true`, `studio.write`.
...
**`studio_list_tokens`** exists because `.studio/framework.json` is Studio's own generated store and it is enormous — 97 KB / ~36,200 tokens in a real project, well past the CLI `Read` tool's 25,000-token ceiling. Agents kept trying to read it whole and kept failing, in the parent turn and again inside each subagent, because nothing is shared between them. The size is not waste: it is ~226 colour tokens at ~420 bytes each, every one carrying its full editor configuration (`id`, timestamps, `generateUtilities`, `generateShades`, …). All of that matters to the framework engine and **none of it matters to an agent choosing a colour**, which needs a name and a value. So this projects rather than truncates — slug plus the resolved value, with the dark value only when the token has one, ~40 bytes instead of ~420, putting the full palette near 2.5k tokens and a filtered query far below that. The prompt states the rule directly: never read `.studio/` yourself, a tool covers each part of it.
...
**Effort persists per project AND per account** (D5, W10) — `.studio/meta.json`'s `agentSession.byUser[<studioAgentUserKey>].effort` (the bare `agentSession.effort` is still READ as the project-wide default for an account that has never saved one, and never written again), round-tripped through `GET/POST /admin/api/ai/studio-session` (`server/ai/handlers/studioAgentSession.ts` — lives under `server/ai/handlers/`, not `server/handlers/studio/`, specifically so it needs no change to `studio.ts`'s own sub-router array). Fidelity mode persists through the same route and the same per-account shape (W9-2); each control is sent on its own request and an omitted field means "leave alone", so the two pickers never overwrite each other. **Permission** mode is never accepted by this route's request schema at all — the same "nowhere to write it" enforcement as the store-level rail above.
...
5. **Stale evidence is elided.** Within one tool loop, only the **most recent** heavy result per tool name (`site_render_snapshot`, `site_read_document`, `site_get_node_html`, or anything with an image) is sent at full fidelity; earlier ones become a one-line breadcrumb (`"Earlier <tool> output removed… Call <tool> again…"`). Older snapshots describe page state the model has since mutated, so they carry no value. This is a **per-request projection** (`projectHeavyElision` in `server/ai/drivers/http/toolLoop.ts`), computed fresh for each POST — the loop's own message history is strictly append-only. It used to rewrite the earlier message in place, which changed the cached prefix on every capture and cost the whole conversation its prompt cache from that point on.
...
**Tool dispatch within a round.** A batch is executed in ordered groups: consecutive `mutates !== true` tools run **concurrently**, and any tool that mutates state (or a name that resolves to no registered tool) gets a group of its own. The system prompt asks the model to issue its reads as one batch, and running that batch serially made a round cost the SUM of every read instead of the slowest one. The conservatism is deliberate — two writes to the same file, or a read the model issued *after* a write in order to observe it, must never be reordered. Stream events are still emitted in the model's own call order regardless of which tool finished first. See `groupToolCalls` in `drivers/http/toolLoop.ts`.
...
**1 — `MAX_TOOL_ROUNDS` (default 40).** A turn may spend at most this many provider rounds. Reaching it emits the aggregated `usage` event (the provider billed every one of those rounds whether or not the turn reached an answer) and then exactly one `error` event that **names the last tool the model called**, so the transcript says what it was looping on rather than simply stopping. Forty is roughly twice the longest legitimate build turn measured here, so a working turn never touches it. Overridable per turn with the request's `maxToolRounds` (1–200, validated by `AiChatRequestBodySchema`).

**2 — repeated mutating calls are answered, not re-executed.** Each mutating call is fingerprinted as `toolName` + its arguments in canonical JSON (key order normalised recursively — `toolCallFingerprint`). The fingerprint set is **per turn**: a write the user asks for again in their next message is a new instruction and runs. The second identical call in one turn never reaches the handler; it is answered with
...
The fingerprint applies only to `mutates === true` tools, the same read/write boundary `groupToolCalls` uses. **Read-only tools are deliberately exempt:** re-reading a document after a write is how a model checks its own work, and that second read is a different question with the same arguments.

The recorded fingerprint covers a FAILED write too. A write that refused for a reason in its own error text refuses identically the second time, and re-running it is exactly the loop the bound exists to stop.
...
- **A turn that never ends (Z3).** The HTTP drivers are bounded by `MAX_TOOL_ROUNDS` (above). The `claudeCli` driver owns no loop of its own — the subprocess does — so it is bounded in wall time instead, by **two differently-shaped timers**:
  ...
  Overridable per turn with the request's `turnCapMs` (1 min – 2 h, validated by `AiChatRequestBodySchema`), on the same session-controls wire as `effort`. Every HTTP driver ignores it, exactly as every non-`claudeCli` driver ignores `effort`.
...
- Source-of-truth files:
  ...
  - `server/ai/drivers/http/toolLoop.ts` — provider loop, its two ceilings (`MAX_TOOL_ROUNDS` and the per-turn `duplicate-call` fingerprint set), grouped (concurrent-read) tool dispatch, heavy tool-result elision as a per-request projection, message cache-breakpoint policy, and one provider-triggered historical-image fallback

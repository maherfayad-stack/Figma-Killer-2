...
|---|---|---|---|
| `studio-outlet` | `base.outlet` | none (the CMS content outlet) | **No** |
| `studio-loop` | `base.loop` | `sourceId`, `filters.tableId`, `orderBy`, `direction`, `limit`, `offset`, `pagination`, `pageSize`, optional `tag` / `customTag` from `data-*` attrs | Yes |

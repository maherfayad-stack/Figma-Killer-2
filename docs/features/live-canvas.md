...
5. The user edits. The inspector's style commit is previewed in the frame at once through `optimistic.style` (`speed-01`); structural gestures run `optimisticDomOps`; the writeback lands in source; Vite's HMR updates the frame; `hmrState.ts` restores input values, focus, scroll and open dialogs by node id.

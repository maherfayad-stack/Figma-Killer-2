...
- **`site_duplicate_node`** (CMS toolset, offered only to an UNBOUND connector) → `store.duplicateNode` → `writeDuplicateToSource` → `deferWhileStructuralCommitInFlight()`. A second call arriving while the first commit is still resyncing is QUEUED rather than posted (`store-14`, replacing `store-11`'s refusal): it re-plans against the tree that resync leaves behind, so two calls are two honest writes instead of two writes against one unshifted original. Serialized.
- **`site_insert_html` / `site_replace_node_html`** → `store.insertImportedNodes`, which merges the imported fragment into the in-memory tree and posts no source write. On a studio-imported tree that used to be a hole of its own shape — the merged nodes carried nanoid ids the next parse deleted without a word. `store-13` closed it: the action now REFUSES the whole fragment on a studio tree (`refuseImportedNodesInto`), returns the reason as the tool's error, and names `studio_apply_edits`' `insert` edit as the path that writes real JSX. `site_replace_node_html` asks before deleting the target's children, so a refused replace never empties the node. See [agent.md → Structure (HTML-native)](agent.md#structure-html-native) for why a source write is not available to route to instead.

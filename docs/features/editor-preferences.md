...
- Storage: `localStorage["studio-editor-prefs"]` (`EDITOR_PREFS_KEY`). `additionalProperties: true` on the schema keeps forward / backward compatibility silent.
...
**`defaultBreakpoint` applies only when the user actually chose it.** Its catalog default is `desktop`, and `readEditorSelectPreference` returns that default for a preference nobody has ever set — so reading it unconditionally meant every load "preferred" desktop and a mobile project could never open on the phone width its screens are drawn at. `applyDefaultBreakpointPreference` now gates on `hasEditorSelectPreference`, which parses the stored prefs against an EMPTY fallback instead of `DEFAULT_EDITOR_PREFS` and can therefore tell "desktop because I picked it" from "desktop because nobody picked anything". An unchosen preference leaves the opening viewport to the project's own shape (`projectDefaultViewport.ts`: the project's default frame width, read through the same `resolveDeviceKind` the device mockup uses, picks the matching viewport — so a 393-wide project opens on its phone). An explicit choice still outranks it.
...
```jsonc
// localStorage["studio-editor-prefs"]
{
  "autoSave": true,
  "hoverPreview": false,
  "theme": "light",

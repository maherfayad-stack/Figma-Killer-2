...
| 🟢 `server/handlers/studio/projectProfileSchema.ts` | Pure schema leaf: `ProjectProfileSchema`/`ProjectProfile`, `ProbeWarning`, `ColorSchemeCapability` (WS-10 §3.1 — `mechanism: 'media'\|'class'\|'none'` + detected `selector`), `LocalesCapability` (WS-10 §4.1 — `keys`/`defaultKey?`/`source`), `DesignSystemRef` (`name`/`source: 'node-modules'\|'imported'`/`root`) — kept separate from `projectProbe.ts` to avoid a load cycle with `studioMeta.ts` (which persists a probe result and is read back by the probe for caching). |
...
| 🟢 `src/core/studio-board/` | `Board`, `BoardFrame`, `StickyNote`, `DocBlock`, `parseBoardsFile`, `devicePresets`, `PreviewAxes`/`DEFAULT_PREVIEW_AXES` (WS-10 Phase 1 — `previewAxes.ts`: `direction`/`colorScheme` render-time, `locale` reserved for Phase 2). Also `MIN_FRAME_SIZE` (`frameGrid.ts`), the shared annotation transforms (`resizeAnnotation`/`reorderAnnotations`/`annotationPaintOrder`, `AnnotationRef`), and `platformPresets.ts` — the mobile/web answer a new project is created with, and the `frameDefaults` it becomes. |
...
| 🟢 `src/admin/pages/site/studio/previewAxesCapability.ts` | WS-10 — client for `GET/POST /admin/api/studio/preview-axes` (persisted `direction`/`colorScheme`/`locale`) + external stores for BOTH the dark-mode `ColorSchemeCapability` and locale `LocalesCapability` probes (`GET /admin/api/studio/probe`, one fetch — `refreshPreviewCapabilities` — refreshes both), same "tiny external store" pattern as `studioProjectTrust.ts`. |

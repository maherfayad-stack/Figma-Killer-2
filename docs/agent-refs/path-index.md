...
| 🟢 `server/handlers/studio/designSystemDigest.ts` | `getOrBuildDesignSystemDigest` — the generated `design-system.md` reference (`agentRoster.ts`'s 7th file): all five token families (colors/typography/spacing/**radius**/**elevation**, the last two `FrameworkSettings` has no home for) plus a one-line-per-component BEM index (class + variants + source file), built from `tokenExtractCssScan.ts`'s shared engine. Cached `.studio/cache/design-system-<hash>.md`, same convention as `styleCompile.ts`. Also exports `computeDesignSystemCacheKey` (stat-based CSS fingerprint) for reuse by `agentRosterManifest.ts`'s regeneration gate. |
| 🟢 `server/handlers/studio/agentRoster.ts` | WS-12 §7 — `generateStudioAgentRoster(dir)`: the ten-agent subagent roster (eleven with an approved Figma-capable MCP server — `figma-asset-scout`) + 6 reference files (7-8 with `design-system.md`/`figma.md`) written to `<project>/.claude/`, called from `claudeCli.ts` before every real chat turn. perf-06: the regeneration fingerprint/manifest mechanics live in `agentRosterManifest.ts` (split out to stay under the module-size budget) — this file owns the roster/reference DEFINITIONS and the write loop, that one owns "should we even bother rebuilding them". |
| 🟢 `server/handlers/studio/agentRosterTypes.ts` | `StudioAgentDef` — the one shared roster-entry shape, split into its own type-only leaf so `agentRosterMcpTools.ts`/`agentRosterFigma.ts` can depend on it without a runtime import cycle with `agentRoster.ts`. |
| 🟢 `server/handlers/studio/agentRosterMcpTools.ts` | mcp-tooling — the fix for the structural blocker that made naming ANY approved external MCP server's tool in a subagent throw and silently degrade a whole turn to zero subagents. `resolveApprovedMcpServerNames`/`assertKnownAgentTools`: a tool name is known when it's a real `studioAgentTools` entry OR `mcp__<server>__<tool>` for a server THIS project has actually approved (read via `../../ai/drivers/{projectMcpServers,registeredMcpServers}.ts` — never a broader guess). Also `mcpServerFingerprintWitness`, folded into `computeRosterFingerprint` so approving/revoking a server forces a full regeneration. |
| 🟢 `server/handlers/studio/agentRosterFigma.ts` | mcp-tooling — `findApprovedFigmaServer` (name/summary heuristic), `figmaReference` (the generated `figma.md` — resolve fileKey/nodeId PER COMPONENT via `studio_list_component_bindings`, check `nodeIdPlaceholder`, land via `studio_fetch_remote_asset`), and `figmaAssetScoutAgent` — the project-conditional eleventh subagent. Its two `mcp__<server>__*` tool names are an honest, unverified assumption (Figma's Dev Mode MCP Server naming) — not dogfooded against a live connection. |
| 🟢 `server/handlers/studio/agentRosterManifest.ts` | perf-06 — `computeRosterFingerprint`/`allOwnedFilesUnchangedSince`/`readManifest`/`writeManifest`: gates `agentRoster.ts`'s regeneration behind a cheap input fingerprint (profile + design-system CSS stat key + roster-definition version + `mcpServerFingerprintWitness`) AND a per-target-file stat check (the clobber-protection half — a hand-edited output file has an unchanged fingerprint but a stat mismatch). Measured ~19ms → ~2-3ms warm (`bun run bench:agent-turn`). |
| 🟢 `server/handlers/studio/agentRosterDocOutline.ts` | `buildDocOutline`/`renderDocOutline` — heading + byte-size outline of a markdown file, what `almosafer-ds-expert` embeds instead of a whole `CLAUDE.md`/`design.md` (the real package's files are ~103 KB/106 KB, over the old 50 KB whole-file cap `DS_FILE_MAX_BYTES` used to silently drop). A small independent copy of `studio_read_package_doc`'s own split logic (`server/ai/mcp/tools/studio/packageDocTools.ts`, out of bounds this round) — `generateStudioAgentRoster` must stay synchronous, that tool's handler is `async`. |

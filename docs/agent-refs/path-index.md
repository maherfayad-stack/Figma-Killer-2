...
| 🟢 `server/handlers/studio/designSystemDigest.ts` | `getOrBuildDesignSystemDigest` — the generated `design-system.md` reference (`agentRoster.ts`'s 7th file): all five token families (colors/typography/spacing/**radius**/**elevation**, the last two `FrameworkSettings` has no home for) plus a one-line-per-component BEM index (class + variants + source file), built from `tokenExtractCssScan.ts`'s shared engine. Cached `.studio/cache/design-system-<hash>.md`, same convention as `styleCompile.ts`. |

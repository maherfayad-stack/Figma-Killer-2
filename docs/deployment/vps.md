...
|---|---|---|---|
| SQLite | `docker compose -f compose.prod.yml -f compose.sqlite.yml -f compose.build.yml up -d --build` | `app` | `workspace`, `data`, `uploads` |
| Postgres | `docker compose -f compose.prod.yml -f compose.build.yml up -d --build` | `app`, `postgres` | `workspace`, `postgres_data`, `uploads` |
| SQLite + TLS | `docker compose -f compose.prod.yml -f compose.sqlite.yml -f compose.tls.yml -f compose.build.yml up -d --build` | `app`, `caddy` | `workspace`, `data`, `uploads`, `caddy_data` |
| Postgres + TLS | `docker compose -f compose.prod.yml -f compose.tls.yml -f compose.build.yml up -d --build` | `app`, `postgres`, `caddy` | `workspace`, `postgres_data`, `uploads`, `caddy_data` |
...
**Upgrading an install created before the `workspace` volume existed?** Its projects are in the app container's writable layer, and the `up -d` that applies the new Compose files deletes them. Copy them out first: [backup-restore.md](backup-restore.md) → "Moving the workspace onto a volume".
...
Replace `DATABASE_URL` with a Postgres connection string for Postgres mode. `STATIC_DIR` must point at the built admin SPA (`dist/` after `bun run build`). The workspace defaults to `./studio-workspace` in the checkout; set `STUDIO_WORKSPACE_DIR` to keep it elsewhere.
...
`docker compose down -v` deletes named volumes. For Studio that means deleting every user's projects (the `workspace` volume), the database and uploaded media. Use it only when intentionally wiping the install.

If you replace the `workspace` named volume with a bind mount, the host directory must be writable by the container's non-root `bun` user: see [backup-restore.md](backup-restore.md) → "Ownership, and bind mounts".

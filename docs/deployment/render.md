...
Render can run Studio from the published Docker image, provide a public web service, attach a persistent disk, provision managed Postgres, and generate stable secret values at deploy time. The checked-in Blueprint templates live under `docs/deployment/render/` and are intended to be copied as the root `render.yaml` in dedicated Deploy to Render template repositories.
...
|---|---|---|---|
| SQLite | `docs/deployment/render/sqlite/render.yaml` | SQLite file | One Render disk mounted at `/app/storage` (DB, uploads, workspace, private data) |
| Postgres | `docs/deployment/render/postgres/render.yaml` | Render Postgres | One Render disk for the workspace, private data, uploads and published artefacts |

Both templates use:

```txt
Image=ghcr.io/corebunch/studio:latest
PORT=10000
UPLOADS_DIR=/app/storage/uploads
STUDIO_WORKSPACE_DIR=/app/storage/studio-workspace
STUDIO_DATA_DIR=/app/storage/.data
STATIC_DIR=/app/dist
STUDIO_SECRET_KEY=<generated by Render>
Health check path=/health
```

Render auto-injects `RENDER_EXTERNAL_URL` (the full public URL) into every web service at runtime. Studio uses it as the CSRF public origin, so no proxy or origin variable needs to be set for a one-click deploy.
...
|---|---|
| `corebunch/studio-render-sqlite` | `docs/deployment/render/sqlite/render.yaml` |
| `corebunch/studio-render-postgres` | `docs/deployment/render/postgres/render.yaml` |

Each template repo README can expose a button:

```md
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/corebunch/studio-render-sqlite)
```
...
The SQLite adapter creates the parent directory for `/app/storage/data/cms.db` on boot. The same disk stores every user's projects under `/app/storage/studio-workspace` (created on boot), the MCP server secrets and Claude CLI logins under `/app/storage/.data`, and uploaded media, fonts, plugin packages, and published disk artefacts under `/app/storage/uploads`.
...
`ipAllowList: []` keeps the database reachable only from Render's private network. The app still needs the persistent disk because Postgres stores content rows, but the Studio workspace and private data live under `STUDIO_WORKSPACE_DIR` and `STUDIO_DATA_DIR`, and uploaded files, fonts, plugin packs, and published artefacts live under `UPLOADS_DIR`.
...
`STUDIO_SECRET_KEY` uses Render's `generateValue: true` support. Operators should copy the generated value from Render's environment settings into their password manager after first deploy. Losing the value means stored AI provider credentials and plugin secret settings must be re-entered and TOTP MFA must be re-enrolled.

`STUDIO_WORKSPACE_DIR=/app/storage/studio-workspace` and `STUDIO_DATA_DIR=/app/storage/.data` are what keep every user's projects, the MCP server secrets and the Claude CLI logins on the disk. Each is a dedicated subdirectory: never set either to `/app/storage` itself (the server refuses to start). Without them they live in the container's filesystem, which Render replaces on every deploy. A service created from a Blueprint that predates the variables must move its projects and private data once, before its next deploy: [backup-restore.md](backup-restore.md) → "Moving the workspace onto a volume".
...
- SQLite template: back up the Render disk mounted at `/app/storage`; it contains `data/cms.db`, `uploads/`, `studio-workspace/` and `.data/`.
- Postgres template: back up the Render Postgres database and the Render disk mounted at `/app/storage`; the disk contains the Studio workspace, the private data, uploaded media, fonts, plugin packages, and published artefacts.
...
| Projects, MCP secrets or the CLI login disappear after redeploy, or the logs show a `[studio:workspace]` warning | `STUDIO_WORKSPACE_DIR` and `STUDIO_DATA_DIR` must point under the mounted disk, e.g. `/app/storage/studio-workspace` and `/app/storage/.data`. Move the current projects before the next deploy: [backup-restore.md](backup-restore.md) → "Moving the workspace onto a volume". |
...
| Adding an AI provider credential or enabling TOTP MFA returns 500 | Confirm `STUDIO_SECRET_KEY` exists and has not been rotated. |
| The service exits at boot with `[studio:workspace] Refusing to start` | `STUDIO_WORKSPACE_DIR` is blank, relative, or overlaps the database, uploads or Studio's code (for example `/app/storage` itself). Point it at `/app/storage/studio-workspace`. |

...
|---|---|---|---|
| SQLite | `docs/deployment/render/sqlite/render.yaml` | SQLite file | One Render disk mounted at `/app/storage` (DB, uploads, workspace) |
| Postgres | `docs/deployment/render/postgres/render.yaml` | Render Postgres | One Render disk for the workspace, uploads and published artefacts |
...
The SQLite adapter creates the parent directory for `/app/storage/data/cms.db` on boot. The same disk stores every user's projects under `/app/storage/studio-workspace` (created on boot), and uploaded media, fonts, plugin packages, and published disk artefacts under `/app/storage/uploads`.
...
`ipAllowList: []` keeps the database reachable only from Render's private network. The app still needs the persistent disk because Postgres stores content rows, but the Studio workspace lives under `STUDIO_WORKSPACE_DIR`, and uploaded files, fonts, plugin packs, and published artefacts live under `UPLOADS_DIR`.
...
`STUDIO_WORKSPACE_DIR=/app/storage/studio-workspace` is what keeps every user's projects on the disk. Without it they live in the container's filesystem, which Render replaces on every deploy. A service created from a Blueprint that predates the variable must move its projects once, before its next deploy: [backup-restore.md](backup-restore.md) → "Moving the workspace onto a volume".
...
- SQLite template: back up the Render disk mounted at `/app/storage`; it contains `data/cms.db`, `uploads/`, and `studio-workspace/`.
- Postgres template: back up the Render Postgres database and the Render disk mounted at `/app/storage`; the disk contains the Studio workspace, uploaded media, fonts, plugin packages, and published artefacts.
...
| Projects disappear after redeploy, or the logs show a `[studio:workspace]` warning | `STUDIO_WORKSPACE_DIR` must point under the mounted disk, e.g. `/app/storage/studio-workspace`. Move the current projects before the next deploy: [backup-restore.md](backup-restore.md) → "Moving the workspace onto a volume". |

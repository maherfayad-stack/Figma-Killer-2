...
- `AssetsPanel` — everything insertable, as a card grid: **Recent**, **Design system** (code-backed components grouped by `module.category`, with `PackageBundleNotice` at its head so an empty list is never silent), **Elements** (`base.*`), **Layouts**, **Saved components**, **Icons** (lazy — the catalogue is fetched on first expand; a click copies the icon's sanitised markup). Every card is a LIVE render of the real component with the props an insert seeds (`AssetPreview`); a design-system component renders inside a shadow root carrying the vendor sheet rewritten by `assetPreviewCss.ts`, so nothing reaches the admin document's cascade. Search is `rankAssets` — name prefix 3, keyword exact 2, name/keyword substring 1.5, description 1 — and a card shows the keyword that matched when the hit did not come from its name. Clicking inserts through `useInsertInserterItem`, the same handler every insert surface uses.

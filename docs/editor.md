...
**Clicking a node pre-focuses it with `preventScroll`.** Every node element carries `tabIndex: 0` for keyboard selection, so a plain click focuses it — and the canvas root is `overflow: hidden` **with real scroll extents**, which is precisely the kind of clipped ancestor a browser scrolls to reveal a newly-focused element. Clicking anything in the lower half of a tall frame yanked the whole board upward by however far that node sat past the viewport (measured: 410px on a single click on an imported screen). `focusNodeWithoutScrolling` in `NodeRenderer`'s `onPointerDownCapture` focuses the element the browser was about to focus anyway, with `preventScroll: true`, so by the time the default action runs there is no focus change left to scroll for. It stands down when the pointer landed on something focusable in its own right (an authored `<input>`, the inline-text editor) — those are the browser's to focus.

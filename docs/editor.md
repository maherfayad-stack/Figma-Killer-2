...
- `ExplorerPanel` — the consolidated navigation panel. Body is gated on `isStudioMode()`:
  - **Studio mode** — no tab row. Renders `StudioExplorer` (`panels/ExplorerPanel/StudioExplorer.tsx`) directly:
    ...
    - `StudioPagesTree` — every page in `site.pages` as a top-level expandable row, stacked in normal document flow (its own `.scroll` is the only scroller in that column — an expanded page's subtree sizes to its own content and never stretches to fill leftover space). Double-click renames via `renamePage`. Its header also hosts `AddFramePicker` (icon-only) — the permanent home for curating a page onto the active board as a frame, since `BoardFramesLayer` only renders that picker in its empty state. Expanding a page reveals its node tree: the ACTIVE page renders the full `DomPanel` in its content-height "embedded" mode (`DomPanel.module.css`'s `.panelEmbedded`/`.treeAreaEmbedded` — no search/insert row, no own scroller); every other page renders `PageLayerSubtree` (`panels/DomPanel/PageLayerSubtree.tsx`) — the same `TreeNode` rendering, lazily mounted only while expanded, resolved against that specific page via `DomTreePageContext`/`selectCanvasPageFor` rather than the active canvas document, and with no live DnD context (dragging is inert there — dnd-kit's `useDraggable` has no activators without a `DndContext` ancestor). Every page row activates its page (`openPageInCanvas`) via `onPointerDownCapture`, mirroring `BoardFramesLayer`'s frame activation, so node interactions inside a non-active page's revealed subtree land on the right page's tree.
  - **CMS mode** — the original shell: one `<Panel>` hosting a top `SegmentedControl` with **Layers / Site / Code / Media** tabs (default **Layers**). Each tab renders the corresponding panel in its headerless `tab` variant — the ExplorerPanel shell owns the chrome (header + tabs + close):
...
- Source-of-truth files:
  ...
  - `src/admin/pages/site/panels/ExplorerPanel/ExplorerPanel.tsx` — consolidated navigation panel; body gated on `isStudioMode()` — studio mode mounts `StudioExplorer` (no tabs), CMS mode owns the shell + `SegmentedControl` (Layers / Site / Code / Media tabs), rendering DomPanel / SiteExplorerPanel (one instance for both Site+Code tabs via `sectionGroup`) / MediaExplorerPanel in their headerless `tab` variant
  ...
  - `src/admin/pages/site/panels/DomPanel/PageLayerSubtree.tsx` — one page's `TreeNode` tree resolved via `DomTreePageContext`, for a page that is not necessarily the active one

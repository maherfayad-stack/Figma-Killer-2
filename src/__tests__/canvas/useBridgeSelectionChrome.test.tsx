...
import type { FrameDocumentAdapter, FrameRuntimeEvent, NodeMeasurement, NodeRef } from '@site/canvas/frameAdapter/FrameDocumentAdapter'
...
function makeFakeAdapter() {
  ...
  const adapter = {
    ...
    setResizeTarget: (ref: NodeRef | null, options: { proportional: boolean }) => calls.push(['setResizeTarget', ref, options]),
...
describe('useBridgeSelectionChrome', () => {
  it('sends the ring tokens once, then the selection, the hover and the resize target through the adapter', () => {
    ...
    expect(calls.find(([name]) => name === 'setResizeTarget')?.slice(1)).toEqual([{ nodeId: textId }, { proportional: false }])
  })
  ...
  it('clears the resize target for a multi-selection, and re-sends it when the scale tool is armed', () => {
    ...
    expect(calls.find(([name]) => name === 'setResizeTarget')?.slice(1)).toEqual([null, { proportional: false }])
    calls.length = 0
    ...
    expect(calls.filter(([name]) => name === 'setResizeTarget').at(-1)?.slice(1)).toEqual([{ nodeId: textId }, { proportional: true }])
  })

/**
 * Animation freezing is scoped to DESIGN frames.
 *
 * The design canvas is a static working surface, so animations there run once and
 * hold their last keyframe. Live mode is a visitor preview — the point of it is
 * seeing the page behave as shipped — so the same frame surface must NOT freeze
 * animations when its interaction model is `'live'`.
 *
 * Asserted through real renders rather than by reading the JSX, because the gate
 * is a `{!isLive && …}` inside the iframe portal and that is exactly the kind of
...
describe('CanvasAnimationInjector mounting', () => {
  it('freezes animations in a design frame', async () => {
  ...
  it('leaves animations alone in a live frame', async () => {
    const page = useEditorStore.getState().site!.pages[0]
    ...
    // Wait for the live iframe's portal to commit — the editor-chrome injector
    // lands in every frame, design or live, so its presence marks readiness
    // without presuming anything about the injector under test.

...
const ALLOWED_ERROR_TOAST_SITES: ReadonlyMap<string, number> = new Map([
  ...
  // An image the server could not land; a PNG the clipboard refused.
  ['admin/pages/site/canvas/useCanvasFileDrop.ts', 1],
...
describe('red-toast policy (P3-A)', () => {
  ...
  it('the canvas write path shows no red card at all', () => {
    ...
    const writePath = [
      ...
      // P3-D — the shared structural commit body moved here from the file above.
      'admin/pages/site/studio/commitStructural.ts',

/**
 * Tests for module `staticPlaceholder` injection into `<studio-hole>` elements.
 *
 * A dynamic module may provide a `staticPlaceholder(props): string` function.
 * At publish time, its output is sanitised and baked into the `<studio-hole>`
...
describe('publishPage — staticPlaceholder injection and sanitisation', () => {
  it('injects the staticPlaceholder output into <studio-hole>', () => {
    const page = makePage({
    ...
    // The placeholder content must appear inside the studio-hole element
    expect(html).toContain('<studio-hole')
    expect(html).toContain('Loading Items')
    // The outer studio-hole wrapper must still be present
    expect(html).toContain('</studio-hole>')
  })
  ...
  it('omits placeholder content when staticPlaceholder is not provided', () => {
    ...
    // The studio-hole must be an empty element (nothing between the tags)
    expect(html).toContain('<studio-hole')
    const holeStart = html.indexOf('<studio-hole')
    const holeEnd = html.indexOf('</studio-hole>', holeStart)

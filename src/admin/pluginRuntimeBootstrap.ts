/**
 * Plugin runtime bootstrap — populates `globalThis.__studio` with the
...
/** The frozen runtime shape we publish on `globalThis.__studio`. */
type PluginRuntime = NonNullable<typeof globalThis.__studio>
...
/**
 * Ensure `globalThis.__studio` is populated. Returns a promise that
 ...
 * the `/runtime/*.js` shims, which read `globalThis.__studio`.
 *
...
async function doInstall(): Promise<void> {
  ...
  if (globalThis.__studio && globalThis.__studio.React !== React) {
    ...
    throw new Error(
      '[@studio/runtime] Detected a second React instance during plugin runtime bootstrap. ' +
      `Host React: ${React.version}; existing React: ${globalThis.__studio.React.version}. ` +
      'Plugin authors must build with `studio-plugin build` so React is externalized.',
    )
  }
  ...
  // overwrite `__studio.hostUi` etc. and substitute components.
  ...
  // type-only namespace shape declared on `globalThis.__studio`. Under
  ...
  globalThis.__studio = Object.freeze(runtime) as PluginRuntime
}

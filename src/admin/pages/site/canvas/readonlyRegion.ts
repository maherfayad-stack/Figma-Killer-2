 ...
 *
 * Read-only composed nodes carry `data-studio-readonly-*` markers; the active
 ...
 * `[data-studio-readonly-*]` ancestor therefore mislabels editable content as
 ...
 * read `data-studio-readonly-label` / `-kind` / `-id` off the returned element.
 */
export function closestReadonlyRegion(target: EventTarget | null): Element | null {
  if (!isElementLike(target)) return null
  const boundary = target.closest('[data-node-id], [data-studio-readonly-label]')

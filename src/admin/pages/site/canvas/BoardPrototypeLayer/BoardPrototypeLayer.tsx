/**
 * BoardPrototypeLayer — the AUTHORED connectors, and the `+` handle that draws
 * them.
 *
 * ## Which half of prototype mode this is
 *
 * Two families of line are drawn over a board in prototype mode, and they are
 * split across two layers because they are different claims that want different
 * geometry:
 *
 *   - THIS layer draws the links the USER AUTHORED. They are anchored to the
 *     ELEMENT the link starts from, because the user placed each one
 *     deliberately, on a specific thing, one at a time — and the `+` handle
 *     that authors them has to be beside that element or the gesture means
 *     nothing.
 *   - `BoardFlowLayer` draws the flows Studio DERIVED from the project's own
 *     navigation code. Those are frame-to-frame: the fact is "Home navigates to
 *     Details", the element is named in the chip's tooltip, and a code flow map
 *     covers every page at once — measuring an element per derived edge is the
 *     cost that would make the whole board a stutter machine. See that file.
 *
 * One store, one mode, one inspector; two layers, because "the link you drew on
 * this button" and "the navigation your repository performs" are not the same
 * sentence and should not be drawn as though they were.
 *
...
export function BoardPrototypeLayer() {
  ...
  // Every store read above is a STABLE reference. Everything derived from them
  ...
  // Only in prototype mode — resolution walks a page tree per link, and design
  // mode never draws a connector.
        ...
        >
          {drawn.map(({ link, route }) => (
            <g key={link.id} className={styles.connector} data-selected={link.id === selectedLinkId ? 'true' : undefined}>
              ...
              <path className={styles.line} d={route.path} />

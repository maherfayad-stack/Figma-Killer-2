...
interface CaptureRenderSnapshotOptions {
   ...
   *     model. Capped so neither edge exceeds ~1568px, matching Anthropic's
   *     own internal downsize.
   ...
   *     total pixel count plus a much higher hard edge ceiling, so a tall
   *     mobile screen keeps its true requested resolution instead of losing
   *     exactness to a vision-safety limit that does not apply to this path.
   *
   * Both caps live in `@core/ai`'s `captureScale.ts` — the headless capture
   * path applies the identical bound, so a verdict never depends on which
   * rasteriser produced the bytes.
   */
...
async function captureElementScreenshot(
  ...
  try {
    ...
    // on an already-large frame). The caps live in `@core/ai`'s
    // `captureScale.ts` so the headless capture path applies exactly the
    // same bound to exactly the same frames — see that module's doc.
    const pixelRatio = effectiveCaptureRatio(
      captureRegion.width,

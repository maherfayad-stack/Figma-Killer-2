/**
 * `studio-plugin init <name>` — scaffold a new plugin project.
 *
 * Creates a minimal but real plugin: one canvas module, a settings entry,
 * and a `studio-plugin.config.ts` that uses the SDK builders. The author can
 * `cd <name>` and run `studio-plugin dev` immediately.
 *
...
function pluginIdFromName(input: string): { pluginId: string; pluginName: string; dirName: string } {
  ...
  if (!trimmed) {
    throw new Error('Plugin name is required: `studio-plugin init <name>`')
  }
...
export async function runPluginInit(
  ...
  if (kind === 'content-editor') {
    ...
    await writeFile(join(pluginDir, 'studio-plugin.config.ts'), pluginConfigTemplate(template), 'utf-8')
...
function serverEntryTemplate({ pluginName }: InitTemplate): string {
  return `import type { ServerPluginModule } from '@studio/plugin-sdk'
...
\`\`\`bash
studio-plugin dev          # watch + sync into the running CMS
studio-plugin build        # produce a .plugin.zip
\`\`\`
...
directory; pass \`--uploads <path>\` (or set \`STUDIO_UPLOADS_DIR\`) when running
outside the studio monorepo.
...
that, every \`studio-plugin dev\` rebuild flows in without another upload.

See [docs/features/plugin-system.md](../studio/docs/features/plugin-system.md)

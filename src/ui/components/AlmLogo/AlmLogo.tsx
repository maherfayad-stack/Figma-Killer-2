 ...
 * single-color glyph the icon set's sizing/color tokens apply to.
 * Used wherever the product needs to show its own identity: the
 * pre-auth screen and the browser favicon (`public/favicon.svg`,
 * generated from the same artwork).
 */
interface AlmLogoProps {
  size?: number
  className?: string
}

export function AlmLogo({ size = 16, className }: AlmLogoProps) {
  return (
    <svg
      width={size}
      height={size}

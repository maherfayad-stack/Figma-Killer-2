/**
 * Registers the VENDORED design system's components (`vendor/alm-design-system/`,
 * imported as `alm-design-system`) as editor modules so they appear in the
 * canvas module inserter, render live on the canvas, are selectable, and get
 * an auto-generated Properties-panel inspector driven by the design-system
 * manifest (prop names + enum values).
 *
 * FIRST SLICE SCOPE:
 ...
 *
 * The manifest is generated by `bun run alm:sync`
 * (`scripts/sync-alm-design-system.ts`; the extraction is Node-only) into
 * `manifest.generated.json`. Besides prop truth it carries the FINDABILITY
 * truth each module publishes here — `description`, `keywords` and `group`
 * (mapped onto `category`, which is what groups and colours a card).
 */
import React from 'react'
import * as DS from 'alm-design-system'
import { withCanvasDrivenProps } from '@site/property-controls/componentPropKind'
...
import { buildDefaults, buildPropsSchema, buildSchema } from './inspectorSchema'
 ...
 * from the design system's own docs, which document props and say nothing
 * about children — same reason `PALETTE_HIDDEN_COMPONENTS` above is a list.
 * These three are the design system's own overlay shells, each documented as
 * taking its panel content as `children` (`CLAUDE.md`, per-component sections).
 ...
 * vendored design system at 1.1.2: the bundle has ZERO `useDir(`
 ...
 * so a design-system node with a composed-children prop this file's old
...
for (const spec of manifest.components) {
  ...
  const mod = {
    id: `alm.${spec.name}`,
    name: spec.name,
    // Description, keywords and category all come from the manifest — the
    // design system's own `design.md` / `CLAUDE.md` plus Studio's curated
    // `studio/{keywords,groups}.json`. The old description was the literal
    // string "<Name> — @alm-design/design-system", which told a person
    // nothing and gave search nothing to match on.
    description: spec.description,
    keywords: spec.keywords,
    // `category` is the Assets panel's SECTION as well as the picker's accent
    // colour — see `moduleAccentForCategory`. Grouping by purpose
    // ("Navigation", "Inputs") is the whole point of the group data; a single
    // flat "Design System" category could only ever sort by name.
    category: spec.group,
    version: '1.0.0',
...
/** `ChevronDownIcon` → `['chevron', 'down']` — the words a search box is actually typed with. */
...
/**
 * The design system's ICON components, discovered from its own runtime exports
 * rather than from `manifest.generated.json`.
 *
 * The manifest is built from the design system's own `CLAUDE.md` / `design.md`
 * (`buildDesignSystemManifest`), which document the 39 public components and
 * say nothing about the icon set. So every `<ChevronDownIcon/>` in a user's
 ...
 * function the vendored package actually exports — so a new icon in a future
 * version registers itself instead of silently regressing to an Unknown-module
 * box. Filtered against the manifest names so a documented component that
 * happens to end in `Icon` is never registered twice with an empty prop schema.
 *
...
for (const name of iconExportNames) {
  const iconMod = {
    id: `alm.${name}`,
    name,
    description: `${name} — design-system icon`,
    // Icons are not in the manifest (they are discovered from the runtime
    // exports above), so their keywords are the facts their name does not
    // already carry: that they are an icon / glyph / symbol, and their name
    // split into words ("ChevronDownIcon" → chevron, down).
    keywords: ['icon', 'glyph', 'symbol', ...iconNameWords(name)],
    category: 'Icons',
    version: '1.0.0',
    icon: CursorClickSolidIcon,
    trusted: true,

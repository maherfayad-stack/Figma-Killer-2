...
| **W8-2** scrub unification (the scrub half; the look pass is separate) | ✅ landed — ONE scrub engine (`useScrubDrag`), consumed by both `ScrubInput` and `ScrubTokenField`, whose duplicate gesture (no rAF coalescing, no min/max, its own ladder) is deleted. Every numeric in the panel now scrubs: TRBL insets, constraint offsets, `gap`, all five corner radii, `opacity`, `zIndex`, `fontSize`, `lineHeight`, `letterSpacing` — plus what already did. Fixed on the way: `line-height: 1.5` was about to become `1.5px` (a different declaration), and glyphless numeric rows never coerced a typed `50` at all. Grid track templates stay excluded on purpose (§5.5). Not dogfooded in a browser — see the `panel-15` handoff in `STATE.md` |

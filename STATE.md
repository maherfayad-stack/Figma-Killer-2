...
### sec-06 — security review of PR #94 (`live-02`, `feat/live-origin-listener`): found and fixed a real SSRF, plus a WS resource-exhaustion gap
...
- **Second finding — unbounded WS resource growth, lower severity:** the pending-message queue (messages arriving before the outbound upstream `WebSocket` finishes its handshake) had no cap, and there was no timeout on the upstream connect itself. A client could flood messages before/instead of ever letting upstream open, growing `ws.data.pending` without bound per connection, or hold the browser socket (and this listener's per-connection state) open indefinitely against a hung/unreachable upstream. Fixed: `enqueuePendingMessage` (exported, pure, directly unit-tested) caps the queue at 1,000 messages / 5,000,000 bytes and closes the socket (code 1013) past either cap; a new `UPSTREAM_CONNECT_TIMEOUT_MS` (15s) watchdog closes the browser socket if the upstream never opens. Timers are cleared on `open`/`close`/`error` to avoid leaking `setTimeout` handles.
- **Everything else on the review checklist passed as-implemented, verified independently, not just by reading the test names:**
  - Cookie stripping (`stripHopByHopAndCookies`/`stripSetCookie`) — read directly, unit-tested against plain `Headers`. WS handshake path never proxies upstream's handshake response headers to the browser at all (`server.upgrade()` issues Bun's own 101 response before the outbound `WebSocket` to upstream is even opened), so a hypothetical upstream `Set-Cookie` on the WS handshake has no path to the browser either.
  - `frame-ancestors` is derived from `config.publicOrigins` (→ `resolvePublicOrigins`), not a duplicated literal; falls back to `'none'` when unconfigured, confirmed by test.
  - `Host` is always rewritten to the upstream's own authority (`stripHopByHopAndCookies(req.headers, upstream.host)`), never the client-supplied `Host`.
  - Cross-project isolation via `projectKey` lookup: confirmed the key is used only as an object-key lookup into the in-memory registry, never concatenated into a filesystem or URL path — traversal segments in `projectKey` itself just fail to match any registered key (fail-closed 404), independent of the SSRF above (which was in `rest`, not `projectKey`).
  - Not-ready projects refuse cleanly (404 `unknown-project` / 503 `not-ready` + phase, no internal detail leaked), on both HTTP and the WS pre-upgrade check (re-checked immediately before `server.upgrade`).
  - Isolation gate (`live-origin-isolation.test.ts`) plus my own independent `grep` for `import(`/`require(`/`from '...'` in `server/liveOrigin.ts`: exactly 3 imports (`@core/utils/typeboxHelpers`, `./config` types, `./handlers/studio/devServer`) — no import of `./router` or `./auth/security`, static or dynamic.
- **Noted but not fixed here — a real design question for L1, not a `live-02` bug:** this listener has **zero access control** beyond a project's dev-server phase being `ready` — by design, per the plan ("no cookies are set or read on this origin"). Today that's inert (`getDevServerStatus` is a stub that always returns `undefined`), but once L1 mints real, `ready` projects, **anyone with network access to `LIVE_PORT` who can guess or learn a `projectKey` can view/interact with that project's live dev server**, regardless of whether they're the project's own admin-authenticated owner — CSP `frame-ancestors` stops *embedding* by a foreign site, it does not stop a direct/curl/raw-WebSocket client. This matters most for self-hosted/tunneled deployments (see the `Tunnel Studio for colleagues` memory note) where `LIVE_PORT` may be reachable by more than the intended user. Recommend L1 mint `projectKey` as an unguessable, per-viewing-session token (not a deterministic function of the project directory name) rather than treating this as solved by "no cookies here" alone. Flagging for `server-engineer`/`studio-architect` to pick up when L1 lands — not a blocker for `live-02` today because the registry is provably empty.
- **Tests added:** `server/liveOrigin.test.ts` — `resolveUpstreamUrl` unit tests (network-path `//`, backslash, and normal-path-plus-query cases), `enqueuePendingMessage` unit tests (count cap, byte cap), and two integration-level adversarial tests through `handleLiveOriginFetch` (`/p/acme-app//evil.example/steal` over HTTP and over the WS pre-upgrade path) asserting the resolved host stays pinned to the real upstream. All new + all pre-existing tests in this file and the isolation gate pass (25/25).
- **Verification:** `bunx tsc -b` clean. `bunx eslint server/liveOrigin.ts server/liveOrigin.test.ts` clean. `bun test server/liveOrigin.test.ts src/__tests__/architecture/live-origin-isolation.test.ts` — 25/25 pass. `bun test server/handlers/studio/__tests__ src/__tests__/architecture` — 890 pass / 2 fail, both pre-existing and unrelated (`icon-catalog-integrity.test.ts` missing a vendored icon file, `bundle-size-budgets.test.ts` `AdminCanvasEditorBody` over budget) — confirmed via `git status`/diff that neither touches anything in this change's scope.
- **Next step:** PR #94 is safe to move out of draft. `studio-architect`/whoever picks up L1 should read the access-control note above before minting real `projectKey`s.
- **Human action needed:** none to unblock this PR. The access-control design question above should be resolved (explicitly accepted or fixed) before L1 ships real dev-server keys.

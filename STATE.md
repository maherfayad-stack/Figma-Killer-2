...
**The complaint that started this: "still a long way to go … a lot of unneeded titles that can be icons".** `panel-07` shipped the glyphs but left every glyph-bearing field ALSO printing its name above itself, and left three sections still speaking the old side-label vocabulary. The panel had two ways of naming one value, and used both.

**The rule is now ordered and lives in exactly one place** (`ClassPropertyRow`'s `resolvedLayout`, documented in `docs/design.md` → "The inspector"): icon toggle group → in-field glyph → self-describing value → caption. **First match wins, and a field that carries a mark does not also get a caption.** If you find yourself adding a caption to a property that has a glyph, the bug is that the glyph is unreadable — fix the glyph or drop it, do not print both.

**`isSelfDescribingProperty` grew `boxSizing`, `color`, `backgroundColor`.** `border-box` and a swatch-plus-hex ARE their own labels. **`background` (the shorthand) is deliberately excluded** — it sits directly under `backgroundColor` in the same section, and two uncaptioned colour-ish fields would be indistinguishable. That exclusion is a judgement, not an oversight; don't "finish" it.

**`ScrubInput.label` is `ReactNode` now.** That is what let `SizeSection` replace "Min W" / "Max H" with the converging (`→|←`) and diverging (`|↔|`) arrow marks, and it keeps drag-to-scrub on the glyph. `aspectRatio` + `boxSizing` pair into one uncaptioned row instead of owning two captioned full-width rows. Size went 8 rows → 4.

**Two more sections gave up their label gutter.** `LayoutSection`'s `LabeledControl` and `BorderControl`'s `FieldRow` both take an **optional** label now and most rows pass none: direction/wrap and align/justify sit side by side (align/justify KEEP captions — the two icon sets are near-identical and the axis is the one thing a picture cannot say); border width and radius carry stroke-weight and corner-radius glyphs; outline/offset pair. New `EffectsSection` pairs the six long-tail effect properties.

**Measured on the ALM fixture (`src/screens/CanonicalScreen.tsx:21:6`, panel at 360px):** Effects 398→257px, Border 404→373px, Typography down to two captions (`White space`, `Text shadow`). `scrollWidth === clientWidth`, so `panel-07`'s clipping fix still holds.

**Accessibility bug found en route, and it predated this pass: `SelectControl` never set an `aria-label`.** It relied entirely on `ControlRow`'s `<label for>`, so every captionless select — `fontWeight` and `fontFamily` since `panel-07`, `boxSizing` since this one — reached assistive tech with **no accessible name at all**. `TextControl` and `ColorControl` had always set one. Now it does too, unconditionally, matching them. **A test that can't find a combobox by name is the symptom.**

**`SectionStylesMenu` is one button per FAMILY, not one per section.** Typography's header carries two (a type mark, a swatch); Background and Border keep one each. A single button opening both families made the Typography menu useless for the thing it existed for: the framework generates a handful of text styles and hundreds of colour utilities, so the six type styles drowned under a heading that scrolled off the top. **The group headings are gone with it** — which family you are applying is settled by the button you pressed.

**Trap for whoever adds a test that seeds `useEditorStore`.** It is a module singleton shared by every test file bun runs in one process. `sectionStylesMenu.test.tsx` seeded it (and stubbed `addNodeClass`) without restoring, and took **five unrelated `propertiesPanel-redesign` tests down with it** — passing alone, failing in a batch. Snapshot the keys you touch and restore them in `afterEach`. The full suite had already been run before those files existed, so the regression was invisible until the batch run.

**Still open (Track G):** `SpacingBoxControl` is now the tallest block in the panel at **253px** and still leads with its box diagram — Figma has no such diagram. `BorderControl`'s side/corner pickers are what is left of its height. Then `AlignGrid`, multi-select style editing, drag-scrub beyond `SizeSection`, the Shift-nudge 10-vs-8 inconsistency, math expressions.

**Files:** `src/ui/components/InspectorIcons/*` (7 new glyphs), `src/ui/components/ScrubInput/ScrubInput.{tsx,module.css}`, `src/admin/pages/site/property-controls/SelectControl.tsx`, `.../PropertiesPanel/{ClassPropertyRow,SizeSection,SectionStylesMenu,cssPropertyIcons,StyleSectionsEditor}.tsx`, `.../PropertiesPanel/EffectsSection.tsx` (new), `.../PropertiesPanel/{SizeSection,SectionStylesMenu,LayoutSection}.module.css`, `.../LayoutSection/{LabeledControl,FlexDirectionControl,FlexWrapControl,GapInput,LayoutSection}.tsx`, `.../BorderControl/BorderControl.{tsx,module.css}`, `src/__tests__/panels/{sectionStylesMenu,sizeSectionDensity}.test.tsx` (new), `src/__tests__/panels/inspectorIconControls.test.tsx`, `docs/design.md`, `STUDIO-FIGMA-PARITY-PLAN.md`.

---

### panel-07 — the properties panel is Figma-shaped now; the label column is gone, and a style can be applied where it is used
- **Agent:** claude (single pass)
- **Stage:** done. `tsc -b`, `bun run build`, `bun run lint` green. Suite triaged against a clean-HEAD worktree: **zero new failures** — every failure in my run was already in the baseline (`comm -23` of the two sorted `(fail)` lists was empty). **Dogfooded in a real browser** (see below), not just gated.
- **Updated:** 2026-08-30

**Track G's density win and the "labels → icons" half of the Figma-parity plan shipped.** Reference was real Figma UI3 captures pulled through the Mobbin MCP (`search_screens`, app "Figma", platform web) and read at 4× crop — the Design tab, the Typography block for a selected text layer, and the page-level Styles list. The Figma MCP cannot serve this: it reads *your* Figma files, and Figma's own product UI is not one of them.

**The one change that mattered most was a default, not a stylesheet.** `ClassPropertyRow`'s `layout` defaulted to `inline`, so every section that wanted density had to opt in at each call site. Half of them never had, and the panel read as two designs stacked — Figma-shaped captions down to one header, then a form with a label gutter under it. Every caller of that component is the properties panel, so the panel's own rhythm is now the default and `inline` is the opt-out. **Symptom to recognise if this regresses: "Aspect ratio" and "Box sizing" showing a label to the LEFT of their field while "Align self" shows one above.**

**What a value is named by, in priority order** (registry: `cssPropertyIcons.ts`, rules: `docs/design.md` → "The inspector"):
1. an icon toggle group and no words (`textAlign`, `fontStyle`, `textDecoration`, `textTransform`) — built as a *skin* on the existing `SegmentedControl`, NOT as the new `IconToggleGroup` primitive the plan proposed; the primitive already existed with icons, tooltips and clear-on-active-click.
2. a glyph inside the field (`lineHeight`, `letterSpacing`, and `SizeSection`'s existing `W`/`H`). **The glyph is `aria-hidden`, so the control must keep the property name as its `aria-label`** — locked down by `src/__tests__/panels/inspectorIconControls.test.tsx`.
3. otherwise Figma's small caption above the field.

An enum only earns a mark when a 14px glyph can genuinely say its values. `whiteSpace`, `overflow`, `objectFit`, `cursor` keep dropdowns on purpose, and there is a test asserting `whiteSpace` is still a combobox — the guard against turning the panel into a rebus.

**Three hardcoded `100px` label columns became one token.** `--control-label-w` + `--control-row-h` in `globals.css`, rebound by the panel root to `--inspector-label-w` / `--inspector-row-h`. Two gate tests asserted the literal and were updated to assert the indirection instead (`ClassPropertyRow.test.tsx`, `PropertyControlRenderer.test.tsx`) — a future density pass should be able to change the value without touching them, but not reintroduce a second copy of it.

**The field skin is opted into per SURFACE, not per control.** The panel root carries `data-field-skin="inspector"`; each primitive's own stylesheet holds a `[data-field-skin='inspector'] .x` block (`Input`, `Select`, `ScrubInput`, `SegmentedControl`). Threading a prop through the ~40 components between the shell and a leaf input would mean every new control had to remember to forward it. **`docs/design.md` §3 previously said "Don't fill them. Don't square the corners." — the exception is now written there. Do not "fix" the skin back.**

**A real clipping bug was found only by driving the browser, and it predated this work.** The scroll column's `scrollWidth` was 326px inside a 257px track, so `overflow-x: hidden` was quietly amputating the right edge of the panel — a clear button with no left half, a truncated "Assign class". Cause: grid items default to `min-width: auto`, and `min-width: 0` on the container does not reach them. Fixed at `.surfaceContent > *`, plus `StyleTargetChip`'s `.row` (now wraps, and `.chipStatic` truncates its own text node — the existing `.chip.chip > *` ellipsis rule targeted child ELEMENTS and the class name is a bare text node). It was there at 360px too, 60px further out of sight.

**Applying a style at the point of use — `SectionStylesMenu`.** A *variable* was already offered inside the field that takes it; a *style* (a framework-generated utility class) was reachable only by name-typing in the ClassPicker. There is now a styles button in the header of the section that owns the family. **Trap:** the generated catalogue is enormous — **4,294 entries for the Typography section on the ALM fixture** — so it ships a search field, a 40-row cap that spends its budget on type styles first, and a count of what it left out. **Second trap: swatches must resolve `var(--…)` through `generateFrameworkColorVariableSets`.** Those custom properties are declared in the CANVAS document, not in the admin chrome the menu renders into, so a raw reference paints nothing and the swatch comes out blank.

**Dogfooding notes for whoever comes next.**
- `bun run dev` serves the app on **:3001 but that port served a STALE bundle** for `.tsx` edits during this session; `http://127.0.0.1:5173` (Vite directly) was live. Two screenshots were read as "my fix didn't work" before this was pinned down — verify against 5173, or check `_propertyRowWrapStacked` is on the row before believing a layout screenshot.
- The login wall is real (`docs`-level note: `STUDIO_BENCH_ADMIN_*` exist for the bench and are unset). A local-only admin `smoke@localhost.dev` already existed in `.tmp/dev.db` from an earlier session; its password was reset to `StudioSmoke!2026` to get in. **Local dev DB only — never propagate.**

**Files:** `src/styles/globals.css`, `src/ui/components/{ControlRow,Section,Input,Select,ScrubInput,SegmentedControl}/*`, `src/ui/components/InspectorIcons/` (new), `src/admin/pages/site/panels/PropertiesPanel/{ClassPropertyRow,StyleSectionsEditor,StyleSurface,StyleRuleComposer,InlineStyleComposer,TypographySection,PropertiesPanel,cssControlTypes}.*`, `.../PropertiesPanel/{cssPropertyIcons.ts,SectionStylesMenu.tsx,SectionStylesMenu.module.css}` (new), `.../PropertiesPanel/{StyleTargetChip,LayoutSection,StackedPropertyGrid,ClassPropertyRow}.module.css`, `src/admin/pages/site/property-controls/{TextControl,TokenAwareInput,shared}.ts*`, `src/admin/state/workspaceLayout.ts`, `src/__tests__/panels/inspectorIconControls.test.tsx` (new), `src/__tests__/property-controls/{ClassPropertyRow,PropertyControlRenderer}.test.tsx`, `src/__tests__/ui/neutralInteractiveChrome.test.ts`, `docs/design.md`, `STUDIO-FIGMA-PARITY-PLAN.md`.

---

### board-10 — every design-system prop got a text box, and `dir` was defaulted onto every component (which defeated board-07)
- **Agent:** claude (single pass)
- **Stage:** done. `tsc -b` clean, `bun run lint` 0/0, architecture 496/1 (the 1 is a committed `margin-left: 20px` in `BoardNotesToolbar.module.css`, not mine). New suites: `almPropControls.test.ts` (6), `slotCandidates.test.ts` (4). **Not dogfooded in a browser.**
- **Updated:** 2026-08-30

**The ask:** icons should be pickable from a dropdown on every module component; props that are really booleans/enums should render as toggles/dropdowns instead of text boxes; and `dir` should follow the canvas LTR/RTL toggle.

**One root cause under the first two.** `src/modules/alm/register.tsx` carried its OWN control mapping — enum with ≥2 values → `select`, **everything else → `text`** — while the rest of the editor (every `pkg.*` component, every local `studio.instance` call site) already went through the shared `controlForPropKind`, which has had `toggle`/`number`/`slot`/drop-the-handler cases all along. ALM ships bundled untyped JS, so `componentSpecExtract.ts`'s `.d.ts` reader returns nothing for it and the manifest recorded `tsType: 'unknown'` for all 317 props; the enum comment was the only signal that survived.

**Fix at the source: the manifest now records a real `kind` per prop**, derived from the VALUE FORM in the package's own documented JSX example — `skeleton={false}` → boolean, `stepperMax={0}` → number, `icon={<SvgIcon />}` → icon, `onClick={fn}` → handler, `bg="default" // a | b | c` → enum. Measured over the 39 components: **100 string, 68 enum, 49 boolean, 32 handler, 19 number, 15 icon, 4 node, 30 unclassified**. An unrecognised form returns `undefined` and falls through to a text box — the honest failure, never a toggle for something that is not a boolean. `register.tsx`'s private mapping is **deleted**; it calls `controlForPropKind` now, and its hand-copied structural twins of `PropSpec`/`ComponentSpec` are gone in favour of `@core/component-manifest`'s real types, so the generator and the consumer cannot drift.
Handler props are dropped from the PANEL only — they stay in `propsSchema`, because a parsed call site legitimately carries `onClick={fn}` and `validateNodeProps` must pass it through untouched.

**`dir` was defeating `board-07`'s provider fix, and nothing said so.** 33 of 39 components document a `dir` prop, and `buildDefaults` stamped `enumValues[0]` — i.e. `'ltr'` — onto **every inserted component**. An explicit prop outranks `DesignSystemProvider`, so the provider `board-07` wired to the frame's direction was overruled by a value nobody chose. `dir` is now in `CANVAS_DRIVEN_PROPS`: stripped from the schema, from the defaults, and from the props handed to the component, so the provider is the only thing that answers `useDir()`. **Deliberate consequence to know about:** a `dir` literal in the user's own source is also stripped on the canvas, so the board's direction axis always wins the PREVIEW. That is what a direction preview means; the source is untouched.

**Icons: the picker was offering the wrong list, not missing a control.** Icon props are node-valued, so they already reached `SlotControl` — whose picker offered ONLY the project's own components (`GET /admin/api/studio/components`). No file a user writes satisfies an icon slot, so `<Cell visual="icon">` kept its reserved square blank unless the source was hand-edited — the empty space in every Account row on the board. New `slotCandidates.ts` builds the picker list from BOTH sources: the design system's icons come **from the module registry** (every `*Icon` export is already registered as a module carrying the `sourceImport` an insert needs), so the picker can never list an icon the canvas cannot render, and a package that adds one appears without a second catalog to sync. Icons lead the list for an icon prop, project components lead otherwise; neither is hidden. The write is the existing, tested `commitStudioInsertSlot` → `insertJsxIntoSlotProp` path — **the candidate now carries its own resolved `importSpecifier`** (bare for a package icon, relative for a project component), because running `relativeImportSpecifier` over `'@alm-design/design-system'` produces a broken relative path.

**Known gap, disclosed not hidden:** the picker offers the **10 named icon COMPONENTS** the package exports. It does not yet reach the **464 raw SVGs** under `src/icons/{line-icons,visual-icons,product-icons}` — those need a `?raw` default import (`designSystemGuide.ts`'s `renderIconReference` documents the form the agent uses), which is a different import shape than `insertJsxIntoSlotProp` writes today. That is the next slice.

**Files:** `src/core/component-manifest/types.ts`, `src/core/design-system-manifest/buildDesignSystemManifest.ts`, `src/modules/alm/{register.tsx,manifest.generated.json}`, `src/admin/pages/site/property-controls/{SlotControl.tsx,slotCandidates.ts}`, `src/__tests__/modules/{almPropControls,slotCandidates}.test.ts`.

---

### board-09 — dark mode worked and still had a white band: the frame's paper was an admin token, and an iframe with no background is transparent
- **Agent:** claude (single pass, follow-up to `board-08` from the user's second screenshot)
- **Stage:** done. **Reproduced and fixed in headless Chromium, with before/after screenshots** — the first visual proof in this thread. Canvas suite diffed against the clean-HEAD worktree: identical 19 pre-existing failures. `bun run build` green, lint clean on my files, architecture 495/2 (both remaining failures are a parallel session's files).
- **Updated:** 2026-08-30

**The symptom after `board-08` landed:** frame content correctly dark, then a hard white band filling the rest of the frame. Dark mode was working; something underneath it was not.

**An iframe document that paints no background of its own is TRANSPARENT.** Nothing in this stack paints `html` or `body` — the publisher reset doesn't, `EditorChromeInjector` doesn't, the framework engine emits only custom properties, and these projects' pages paint their own `<main class=page>`. So the frame's visible background is the **embedding element**: `.iframe` (and `.viewport`) in the PARENT document, which wore `background: var(--overlay)` — an admin-theme token, `#ffffff`. And the uncovered area is large, not incidental: `CanvasScrollUnrollInjector`'s blanket `min-height: auto` deliberately kills `.page`'s `min-height: 100%`, so the page collapses to content height while `body` stays pinned to the device viewport and `.iframe` keeps `min-height: 100vh`.

**Reproduced exactly in Chromium before changing anything** (vendor CSS + the project's real page CSS + the actual `applyIframeBodyReset` writes, in a nested iframe): white band with a light paper, dark band with a dark one. Two things this ruled out, both of which looked plausible and are wrong: the UA `color-scheme` canvas is NOT the culprit (it paints correctly when it applies), and `background: Canvas` + `color-scheme` on the iframe ELEMENT does NOT fix it — measured, screenshotted, discarded.

**Fix:** two new tokens, `--canvas-frame-paper` / `--canvas-frame-paper-dark` (the dark one is `#121212`, the UA's own dark canvas, so the paper matches what a browser paints behind an unstyled dark document), selected by `data-preview-scheme` on the frame element from its own effective axes. **They are deliberately THEME-INDEPENDENT** — same reasoning as the `--note-*` tokens: this surface stands for the PREVIEWED page's canvas, so it follows the board's colour-scheme axis and must never follow the admin's theme. `BreakpointFrame`'s wrapper sits outside the iframe portal and so cannot read `FramePreviewAxesContext`; it uses the new `useResolvedFrameAxes(axesOverride)`, which is the same board+override merge.

**`IframeFrameSurface.tsx` is at exactly 700 lines again** — the ceiling, passing but with zero headroom. `board-07` already extracted `CanvasFrameContexts.tsx` out of it for the same reason. The next addition to this file must extract something first; do not grandfather it.

**Files:** `src/styles/globals.css`, `src/admin/pages/site/canvas/{IframeFrameSurface.tsx,IframeFrameSurface.module.css,BreakpointFrame.tsx,BreakpointFrame.module.css,previewAxesFrameEffect.ts}`, `src/__tests__/canvas/previewAxesFrameAttributes.test.tsx`, `docs/agent-refs/{canvas-internals,path-index}.md`.

---

### board-08 — dark mode still rendered fully light: the token extractor flattened the design system's aliases and the copy outranked the original
- **Agent:** claude (single pass, follow-up to `board-07` from the user's screenshot)
- **Stage:** done. `server/handlers` suite 1055 pass / 1 pre-existing fail; new regression tests in both extractor suites; lint clean on my files. **Verified end to end in tests, not in a browser.**
- **Updated:** 2026-08-30

**`board-07` fixed three real layers and the canvas still previewed light.** The user's screenshot — dark selected, `previewAxes.colorScheme: "dark"` persisted in `.studio/meta.json`, both frames rendering white — was a fourth defect underneath, and it is the one that actually paints the pixels.

**The mechanism, in order.** The project's pages style themselves with the design system's SEMANTIC tokens (`background: var(--background-base-default)`). ALM declares those exactly once, as aliases over its raw palette — `--background-base-default: var(--color-light)` — and flips only the RAW palette in its dark block (`:root[data-theme=dark]{--color-light:#1c1c1c}`). The alias is what carries dark mode into every semantic token.
Studio's token extractor decided a token had a dark value **iff the dark block re-declared that token's own name** (`dark.get(name)`). Aliases are never re-declared, so they were imported as light literals with no dark value: measured against the real package, **27 of 171 colour tokens** got a dark value, and **not one of the semantic tokens a page actually references**. Those literals are then emitted by the framework engine at `:root` inside `@layer user-authored` — 4,319 custom properties for this project — and **`@layer user-authored` outranks `@layer vendor` regardless of specificity**, so the vendor bucket holding the package's own correct `:root[data-theme=dark]` palette could never win. Studio was shadowing the design system with a frozen, light-only copy of it.

**The fix is one line in each of two copies of the same logic:** resolve the token's OWN declaration against the merged light+dark map (`resolveVarValue(dark.get(name) ?? raw, darkMerged)`) instead of only reading re-declarations, and keep a dark value only when it actually differs from light. `tokenExtractCssScan.ts`'s `classifyCssText` (the `tokens` route) and `designImport/parseCssTokens.ts`'s `buildCssCandidates` (the "Import design tokens" wizard — the path this project's store actually came from) both had it; their docs already said the two must agree. **27 → 126 of 171** tokens now carry a dark value, and the 45 that still don't are correctly static (`-static` suffixes, brand lockups). Proven end to end: vendor CSS → `classifyCssText` → `buildFrameworkSettings` → `generateFrameworkRootCss` now emits `--background-base-default`/`--text-base-default` inside the `html[data-studio-scheme='dark']` block.

**One existing gate asserted the bug and had to be corrected, not worked around.** `tokenExtract.test.ts` asserted `brand-secondary` (declared `var(--brand-primary)`, where `--brand-primary` HAS a dark override) must end up `darkModeEnabled: false`, commented "must not fabricate one". That is not fabrication — in any browser that token IS the dark value under `[data-theme=dark]`. The comment confused "the dark block names this token" with "this token resolves differently in dark", which is exactly the confusion that broke the design system. The assertion now reads `darkValue === '#7799ff'`.

**An already-extracted project does NOT heal itself, by design.** `mergeExtractedFramework` uses a whole-family never-clobber rule: once `colors.tokens` is non-empty it is never touched again, so a prior extraction's output survives a re-scan the same way a user's hand-authored palette does. `studio-workspace/untitled` and `untitled-2` were therefore **re-extracted explicitly, at the user's request**, by running `extractProjectTokens` + `writeStudioFrameworkFile` directly and deliberately bypassing the merge (`preferences` carried over; the previous file kept beside it as `.studio/framework.json.pre-board08.bak`). Both went **226 tokens / 27 dark-enabled → 171 / 126**. The 55 dropped tokens came from the wizard's `styles/imported/` copy rather than the package's real `dist/index.css`; **checked before accepting the drop — none of them is referenced by either project's source, and every token either project references is defined by the vendor CSS, present in the store, or carries an inline `var(--x, fallback)`.** Emission verified: `--background-base-default` now resolves white → `#1C1C1C` and `--text-base-default` `#1C1C1C` → near-white inside the `html[data-studio-scheme='dark']` block.
**Any OTHER project extracted before this fix still holds a light-only store and must be re-extracted the same way.** If a future task wants "Re-scan tokens" to refresh in place, the missing piece is a per-token "extracted and never edited" marker; `origin` is not it (every token in the old stores read `studio-authored`).

**A defect in the generated pages, found while proving this, now fixed.** `untitled-2/pages/{Home,Page}.module.css` referenced `var(--text-primary-default)` and `var(--text-secondary-default)`. **Neither token exists in `@alm-design/design-system`** — the real names are `--text-base-default` / `--text-base-subtext` — so those declarations resolved to nothing and text fell back to inherited black, which would have stayed black once the background flipped dark. Repointed at the real tokens. (`untitled` already used the correct names.) An agent regenerating screens for this package should read `studio_list_tokens` rather than inventing a plausible-sounding name.

**Files:** `server/handlers/studio/tokenExtractCssScan.ts`, `server/handlers/designImport/parseCssTokens.ts`, `server/handlers/studio/__tests__/tokenExtractCssScan.test.ts`, `server/handlers/designImport/__tests__/parseCssTokens.test.ts`, `server/handlers/__tests__/tokenExtract.test.ts`, `docs/features/canvas-iframe-per-frame.md`.

---

### board-07 — RTL only reached half of every design-system component, and "light" rendered the vendor palette dark
- **Agent:** claude (single pass)
- **Stage:** done. `bun run lint` 0 errors / 0 warnings; canvas + board suite triaged against a clean-HEAD worktree (**identical** 19 pre-existing failures, diff of the sorted lists empty); `server/handlers` + agent/ai suites 1522 pass vs. the baseline's 1517, same single pre-existing failure. **Not dogfooded in a real browser** — the admin login blocks it here, same as `board-04`.
- **Updated:** 2026-08-30

**Follow-up to `board-04`'s "audit/fix RTL + light/dark from an NPM design system".** That entry fixed the `:root` trap in `darkSchemeCssTransform.ts` and stopped there. Three further defects were underneath, each one a different layer of the same question, and none of them was in the CSS rewrite.

**1. Every design-system component was pinned LTR, in JS, while its CSS flipped.** `src/modules/alm/register.tsx` and `studio/registerProjectModules.ts` both wrap each component in the package's own provider — with `React.createElement(Provider, null, inner)`, i.e. **no props**. `@alm-design/design-system`'s every component resolves direction through `useDir(prop)`: explicit prop > `DesignSystemProvider` context > a built-in `'ltr'`. So the board's RTL toggle flipped `html[dir]` (driving all 40 `[dir=rtl]` rules in the package stylesheet) and left every mirrored chevron, direction-aware label and platform-aware layout facing left-to-right. Half-mirrored screens, which read worse than either whole direction.
   Fixed with `FramePreviewAxesContext` (`previewAxesFrameEffect.ts`), published per FRAME — not per board, because a "duplicate as variant" frame previews RTL beside an LTR board — and consumed by both registration paths, which pass `dir` into whichever provider they found. **Do not "simplify" this to reading `document.dir`:** a React component cannot, and per-frame overrides are real. Regression test: `src/__tests__/canvas/designSystemPreviewDirection.test.tsx`, which asserts the `dir` attribute ALM puts on each component's own root — the visible end of frame axes → context → provider → `useDir()` → DOM.

**2. Previewing LIGHT rendered the vendor palette DARK.** `applyPreviewAxesToFrameDocument` set the detected attribute for dark and **removed** it for light. Absence is not neutral: `@alm-design/design-system`, which `ProjectCssInjector` injects into EVERY frame, declares its light tokens under `:root:not([data-theme=light])`, so an unset `data-theme` resolves to the dark palette. Exactly the same broken-in-both-directions shape as the `prefers-color-scheme` bug one layer up. `data-theme` (`VENDOR_THEME_ATTR`) is now written explicitly in both schemes, unconditionally, and an attribute-mechanism gate is set to `light` rather than removed. **A class gate still comes off for light — `.dark` off IS light; only an attribute has a light value to write.**
   `ProjectCssInjector` was ALSO pinning `data-theme="light"` in a mount effect, written before the board had a dark-mode control. Two writers of one attribute is how the light case silently stopped working once the toggle existed; that pin is deleted and `previewAxesFrameEffect.ts` is the single owner of every root attribute the axes drive.

**3. The dark-mode probe was structurally blind to the design system.** `detectColorScheme` walked `listWorkspaceFiles`, which excludes `node_modules` by policy. A project that expresses no dark mode of its own **because its design system expresses all of it** reported `mechanism: 'none'` — so the toolbar's dark toggle rendered DISABLED with "no dark-mode stylesheet was detected", and `studio_project_profile` told the agent the same. Both false about a project whose every component is dark-capable. `colorSchemeDetect.ts` now runs two ordered passes (project's own stylesheets, then its installed component packages' — conventional output dirs only, never a recursive walk) and reports a `source` naming the winning file, which the toolbar tooltip and the agent both surface.

**`PROBE_VERSION` is new, and it is not "re-probe when a field is missing".** A cached `.studio/meta.json` profile is only re-probed when its `probeVersion` is BELOW the current one — a positive statement that the probe's logic has since learned to find something this cache could not, so the cache is provably wrong rather than merely old. Adding a schema field is still not a staleness reason (see `isProfileStale`'s doc, which now says both). Without this, every already-imported project would have kept its wrong `colorScheme: {mechanism:'none'}` forever.

**Agent knowledge, which was the third part of the ask.** The system prompt's direction/colour-scheme section now says HOW the canvas drives both (frame root attributes; the provider for JS-resolved direction), that writing `dir=` on a component call site pins it out of the board's control, and that absence of a theme attribute is not "light". The `studio://guidelines` MCP resource gained a matching section (§7, renumbering the two after it), and `studio_project_profile`'s description now names `profile.colorScheme`'s `mechanism`/`selector`/`source` — the last being the field that tells an agent the gate lives in the design system, not in any file it can grep.

**One refactor this forced (do not undo).** `IframeFrameSurface.tsx` was sitting at exactly 700 lines, the module-size ceiling. The three contexts a mounted frame publishes — its `<iframe>`, its `Document`, its effective `PreviewAxes` — moved to `CanvasFrameContexts.tsx`. "Which contexts does a frame publish" is a separate reason to change from "how is the iframe mounted, measured and wired for events", and a fourth one now costs that file nothing.

**Not mine, present in the tree while I worked:** a parallel session's in-flight duplicate `sourceIntrinsic` blocks in `src/core/module-engine/types.ts` and `src/modules/base/{container,text}/index.ts` (breaks `tsc -b`), `nodeActions.ts` over the size ceiling at 701 lines, and a committed `margin-left: 20px` in `BoardNotesToolbar.module.css` that fails `admin-spacing-token-policy`. All three are outside my diff.

**Still not verified in a browser** (`board-04`'s open item, unchanged): whether a real RTL/dark canvas LOOKS right. Everything here is proven in headless tests against the real installed `@alm-design/design-system`, not by eye.

**Files:** `server/handlers/studio/{colorSchemeDetect,projectProbe,projectProfileSchema}.ts`, `server/handlers/__tests__/{projectProbe,appRoot}.test.ts`, `server/ai/tools/studio/systemPrompt.ts`, `server/ai/mcp/{resources.ts,tools/studio/projectTools.ts}`, `src/admin/pages/site/canvas/{previewAxesFrameEffect.ts,CanvasFrameContexts.tsx,IframeFrameSurface.tsx,ProjectCssInjector.tsx}`, `src/admin/pages/site/canvas/__tests__/previewAxesFrameEffect.test.ts`, `src/admin/pages/site/studio/{previewAxesCapability.ts,registerProjectModules.ts}`, `src/admin/pages/site/toolbar/PreviewAxesControls.tsx`, `src/modules/alm/register.tsx`, `src/__tests__/canvas/{designSystemPreviewDirection.test.tsx,previewAxesFrameAttributes.test.tsx}`, `docs/agent-refs/{canvas-internals,path-index}.md`, `docs/features/canvas-iframe-per-frame.md`.
---
...
### board-11 — the icon picker offered ten chevrons for a 568-icon design system, `dir` was still stamped by the OTHER registration path, and four `dir="ltr"` literals had already been written into the user's source
- **Agent:** coordinator (direct)
- **Stage:** implemented, gates green (`tsc -b`, `bun run lint`, architecture 496/1 — the 1 is the pre-existing `BoardNotesToolbar.module.css` `margin-left: 20px`). **Not dogfooded in a browser** — the admin login still blocks it; the panel behaviour is the user's check.
- **Updated:** 2026-08-30

Follow-on to `board-10`, driven by a screenshot showing a `dir` row reading `ltr` on an RTL board. Four things, all downstream of the same root cause: a rule that lived in ONE registration path instead of the shared layer.

**1. `board-10`'s `dir` rule reached only `alm.*`.** `registerProjectModules.ts` — the GENERIC path every design system other than the one hardcoded package goes through — had no `CANVAS_DRIVEN_PROPS` at all: it stamped `enumValues[0]` (`'ltr'`) into `defaults`, spread it onto the component, and gave it a panel row. The exact defect `board-10` fixed, fully alive for any other project. The rule now lives in `componentPropKind.ts` (`CANVAS_DRIVEN_PROPS`, `isCanvasDrivenProp`, `stripCanvasDrivenProps`) beside the `PropKind → PropertyControl` mapping both paths already shared, and `controlForPropKind` itself now returns `undefined` for a `handler` — so `register.tsx`'s private `isEditableProp` filter is gone rather than copied. Gated by `canvasDrivenProps.test.ts`, which scans BOTH registration files: the failure mode is a path that forgets the rule, not a wrong branch inside one function.

**The deliberate boundary:** the rule applies only to the two paths that WRAP the component in the design system's provider. A local component's call site (`componentCallSiteRows.ts`) keeps its `dir` row — Studio supplies no provider there, so hiding it would remove a control with nothing left to drive it. Pinned by a test.

**2. Studio had already written `dir="ltr"` into the user's source** — four call sites across `untitled-2/pages/{Home,Page}.tsx`, stamped by the pre-`board-10` defaults and saved back through the codemod (the tell is the formatting: `showSeparator dir="ltr"`, appended to an existing line). The canvas now strips `dir` at render, so the board looked right while the user's real app stayed pinned LTR — and with no panel row left, there was no way to see or fix it from Studio. Removed from the source. **A future agent should not "restore" them.**

**3. The icon picker offered the wrong set.** `slotCandidates.ts` sourced design-system icons from the module registry — every `*Icon` export — which for `@alm-design/design-system` is TEN glyphs: the chevrons, checkmarks and radio buttons its own components draw with. The real set is 568 SVG FILES under `src/icons/`, published deliberately (`package.json#exports` maps `"./src/icons/*"`), none of them a React export, so no registry lookup could ever see them. New `GET /admin/api/studio/icons` (`server/handlers/studio/iconCatalog.ts`) reads them off disk; the file catalogue now LEADS an icon prop, with the icon components and the project's own components below it. Nothing is hidden — it is an ordering, not a filter.

**Why markup rides along with the metadata:** every consumer needs it immediately (the picker previews, the pick writes), so a metadata-only list would force a fetch per visible row and still not show the user what they were choosing. Affordable only because of the 4 KB per-file cap, which takes 402 of the 568 files and excludes exactly the ones that should never be inlined — the country flags run to 93 KB. Cached client-side per project, fetched on the first "Add" click.

**4. How an icon is WRITTEN, and why not an import.** `import icon from '…/wifi.svg'` is not a write Studio can honestly make: what it evaluates to depends entirely on the user's bundler (a URL string under Vite's default, a component only with `vite-plugin-svgr`, a compile error under plain `tsc`) — Studio would be writing source whose meaning it cannot verify into a project whose build it does not own. So `svgToJsxNode.ts` converts markup into the JSX subtree the existing slot codemod already accepts (`SlotJsxNodeSchema` was ALREADY recursive with props + children — the write path needed no server change at all). Inline JSX means the same thing in every React project, needs no import or plugin, and round-trips through Studio's own pipeline: `serializeInlineSvg` → `base.svg` → rendered on canvas. Proven end-to-end by a test that takes a real installed ALM icon, converts it, and runs the REAL codemod against a fixture file — asserting `strokeLinecap` is camelCased, `stroke-linecap` is absent, and no import was added.

**SVG upload rides the same path** (`SlotPicker.tsx`): a file the user picks is read in the browser, sanitised, converted and inlined. Nothing is stored, nothing lands in `uploads/`, and there is no file for a later build step to resolve. Sanitisation happens client-side because it has to: `sanitizeSvg` needs a DOM, Bun has none, and calling it server-side would silently return `''` for every icon.

**Not built, and the honest reason:** **lucide (or any third-party icon library) is not offered.** Reaching it means either adding a dependency to the USER's project (a real install job + a `package.json` they did not ask to change) or vendoring a second icon set into Studio — a different decision from "show the icons your design system already ships", and one the user should make explicitly. The upload path covers a one-off lucide glyph today: download the SVG, upload it, and it inlines like any other.

**Files:** `src/admin/pages/site/property-controls/{componentPropKind.ts,slotCandidates.ts,SlotControl.tsx,SlotPicker.tsx,svgToJsxNode.ts,controls.module.css}`, `src/admin/pages/site/studio/{iconCatalog.ts,studioSaveRequests.ts,registerProjectModules.ts}`, `src/modules/alm/register.tsx`, `src/admin/pages/site/panels/DependenciesPanel/useDependencyInstallJob.ts`, `server/handlers/studio/iconCatalog.ts`, `server/handlers/studio.ts`, `docs/agent-refs/path-index.md`, tests `server/handlers/__tests__/iconCatalog.test.ts` + `src/__tests__/{modules/canvasDrivenProps,modules/slotCandidates,property-controls/svgToJsxNode}.test.ts`, and the user's `studio-workspace/untitled-2/pages/{Home,Page}.tsx`.

### mcp-tooling-a2 — studio_compare's node-mapping used the wrong pixel space at dpr!=1, and A2's "region-scoped compare" framing was replaced with a capture-purpose split
- **Agent:** mcp-tooling
- **Stage:** implemented, tests added and green, `bun run build` + `bun run lint` clean.
- **Updated:** 2026-08-30

Two related fixes to `studio_compare`'s fidelity ruler, both in `server/ai/mcp/tools/studio/frameDiffEngine.ts` / `compare.ts` and their MCP-tool neighbors.

**FIX 1 — region→node mapping was scale-mismatched at dpr!=1.** `computeFrameDiff` intersected a differing REGION (in the diff image's pixel space — CSS px × capture dpr) directly against `nodeRects` (always plain CSS px, `getBoundingClientRect()` output, never scaled). At dpr:1 this coincidentally worked; `studio_compare` routinely captures at dpr 2-3 to match a Figma export's own width, so at dpr:2 only the top-left quadrant of a frame could ever map to a node — the agent was being pointed at the wrong nodes on nearly every real compare. No existing test exercised dpr != 1.

Fixed by making the coordinate space part of the type, not a convention: `FrameDiffOptions.nodeRects` is now `{ rects: readonly NodeRect[], imageScale: number }` — `imageScale` has no default and the field cannot be supplied without it, so a future caller cannot reintroduce this bug by forgetting a multiply. `studio_export_frames`' response gained a matching `imageScale` (derived from the REAL captured pixel size ÷ CSS width in `studioExportFrames.ts`, NOT the requested `dpr`, so it stays correct even when the effective ratio was clamped by A2's own caps below); `compare.ts` threads it straight into `computeFrameDiff`. `studio_diff_frames` (the external-caller-facing sibling, `diffFrames.ts`) gained the equivalent `nodeRectsImageScale` input — kept OPTIONAL there, defaulting to 1, specifically to not break that tool's already-shipped dpr:1 contract (its own existing test asserts unscaled mapping still works); a caller now has an explicit way to supply the right scale via the new `imageScale` `studio_export_frames` returns.

Regression tests: `frameDiffEngine.test.ts` (a dpr:2-shaped 400x400 diff image where a defect lives ONLY in the lower-right quadrant of image space but the CSS-px node rect lives in the corresponding lower-right CSS quadrant — fails pre-fix, passes post-fix) and `diffFrames.test.ts` (same shape, through the MCP tool's `nodeRectsImageScale` input, plus asserting the OLD unscaled default still behaves as before for backward compat).

**FIX 2 — A2's "region-scoped compare (crop to a viewport-height slice)" was rejected; a capture-purpose split shipped instead.** The plan's own proposed fix could not have worked: the ~1568px `MAX_IMAGE_EDGE` clamp fires at CAPTURE time (`renderEvidence.ts`'s `captureElementScreenshot`, before `html-to-image`'s `toCanvas()` rasterizes), not at diff time — slicing the diff image afterward cannot recover resolution that was never captured. The real fix was recognizing the clamp exists for a reason `studio_compare` never needed: 1568 matches Anthropic's own internal downsize of an image actually shown to a model (`AI_USER_IMAGE_MAX_EDGE`, `src/core/ai/userImage.ts`), and `studio_compare` diffs server-side with pixelmatch, controlling model visibility separately via `includeImages` (A4 — already shipped, contrary to the plan's old status table, which is now corrected).

Shipped: `captureAgentRenderSnapshot`/`captureElementScreenshot` gained `purpose: 'vision' | 'measurement'` (default `'vision'`, so every existing caller — `site_render_snapshot`, an external `studio_export_frames` caller who didn't opt in — is byte-for-byte unchanged). `'vision'` keeps the old MAX_IMAGE_EDGE=1568 per-edge clamp. `'measurement'` is bounded by a TOTAL PIXEL BUDGET (`MEASUREMENT_MAX_PIXELS = 15,000,000` ≈ 120MB of transient canvas + PNG-encode memory — html-to-image's `toCanvas()` backs an RGBA buffer at 4B/px, `toDataURL` holds roughly a second same-size buffer while re-encoding) plus a much higher hard per-edge ceiling (`MEASUREMENT_MAX_EDGE = 4096`, the well-known cross-engine-safe canvas dimension figure — WebKit has historically been far more restrictive than Chromium/Gecko). **The budget is deliberately set BELOW the edge ceiling squared (~16.8M px)** — set it above and the edge ceiling alone always wins, silently degrading `'measurement'` into "a per-edge clamp with a bigger number," exactly the shape of cap A2 exists to not be. `StudioExportFramesInputSchema` gained the matching `purpose` field; `compare.ts` requests `purpose: 'measurement'` on its own internal capture. `describeResampleReason`'s "likely vision-cap" heuristic was a loose `baselineHeight >= 1567` check with NO upper bound — harmless while `'vision'` was the only mode (a taller baseline was then impossible) but a real false-positive risk once `'measurement'` captures can legitimately be much taller; tightened to a ±2px band around the actual cap so it stops telling a false story about why a resample happened.

Verified composition end-to-end in a new `compare.test.ts` (previously did not exist) against a real temp Studio project (`createScaffoldedPage` + a hand-set board frame size + `registerDesignReference` — only `awaitEditorBridgeForUser`/`awaitStudioLiveReload` are mocked, everything else — dpr math, `reconcileReference`, `computeFrameDiff` — runs for real): **a 390×844 mobile frame measured against its own 2x Figma export (780×1688) now requests `dpr:2` + `purpose:'measurement'` and lands `capture.dimensionMatch: 'exact'` with `similarityScore: 100`** — previously this would have hit the height clamp and fallen into the `resampled` fallback. Also covers worst-region node-id mapping through `imageScale`, and pass/fail verdict gating (structural region vs. thin-spread failure vs. a clean pass).

**Files:** `server/ai/mcp/tools/studio/{frameDiffEngine.ts,compare.ts,diffFrames.ts,exportFrames.ts}` + new `compare.test.ts`, `server/ai/mcp/tools/studio/frameDiffEngine.test.ts` (extended), `server/ai/mcp/tools/studio/diffFrames.test.ts` (extended), `src/admin/pages/site/agent/{renderEvidence.ts,studioExportFrames.ts}`, `src/__tests__/agent/renderEvidence.test.ts` (extended), `src/core/ai/toolSchemas.ts` (`StudioExportFramesInputSchema` gained `purpose`), `STUDIO-FIGMA-PARITY-PLAN.md` (A2 entry rewritten to describe what shipped and why the crop framing was rejected; status table corrected).

**Not touched, per the work order's explicit boundary:** `server/ai/tools/studio/{agentToolNames.ts,index.ts,systemPrompt.ts}` and `server/ai/mcp/tools/studio/typecheck.ts` — reserved by a concurrent session. `src/core/ai/userImage.ts` was read but not modified — `AI_USER_IMAGE_MAX_EDGE` stayed the single source of truth for the `'vision'` cap; the `'measurement'` caps live in `renderEvidence.ts` (the only place that needs them) rather than being force-fit into that file's declared "user-image ingestion" scope.

**Not done / next:** A3 (reference-free quality signals) and A7 (Figma discoverability) remain open per the plan. A5/A6/A8 already landed per the existing status table, unaffected by this change.

### board-12 — a filled slot showed its internal sentinel behind a padlock, the picker rendered at label width, and data binding is gone from the editor
- **Agent:** coordinator (direct)
- **Stage:** implemented; `tsc -b` and `bun run lint` clean. **Not dogfooded in a browser.**
- **Updated:** 2026-08-30

**1. A slot became unusable the moment it was FILLED.** `PropertyControlRenderer` swaps any row for a read-only `CodeValueControl` when `sourceLockReason` is set or the value is structured. Both conditions ask *"can a scalar write land on this attribute?"* — the right question for a text box and the wrong one for a `slot`, which never writes the attribute value at all: "Add" runs the `insert-slot` codemod against the CALL SITE (with its own `explainStructuralConstraint` check), and "Edit contents" merely selects a node that already exists. A JSX-valued prop is code, not a writable literal, so `propLockReason` returned `'set in code'` and the row rendered a padlocked code value displaying the raw `studio-slot:<nodeId>` sentinel — an internal marker no user should ever see — in place of the affordance for the icon just inserted. `slot` is now exempt from that early return. **Empty slots worked and filled ones did not**, which is why this survived until someone actually inserted an icon.

**2. The picker rendered at label width.** `ControlRow` is a two-column grid (`--control-label-w` + `1fr`); the picker is its THIRD child, so it wrapped to a new implicit row and landed in the LABEL column — which the properties panel narrows to `--inspector-label-w`. Icon names truncated to `airl…` and the "showing N of M" line broke one word per line. `grid-column: 1 / -1`, the same span the sibling `.description` already uses. List max-height 160px → 260px: five visible rows is a keyhole for a catalogue of hundreds.

**3. Data binding ("Bind to data field") removed from the editor.** Deleted: `property-controls/DynamicBindingControl/` (control, popover, cache, helpers, barrel), `bindingCompatibility.ts`, the `dynamicBinding` prop and render branch in `PropertyControlRenderer`, the wiring through `renderModuleTabContent`/`PropertiesPanel`/`usePropertiesPanelData` (including `resolveEnclosingLoopContext`/`extractLoopTableId`, which existed only to feed the picker's field list), `ParamPromotableRow`'s "dynamic binding takes precedence" branch, and the store actions `setNodeDynamicBinding`/`clearNodeDynamicBinding` with their `types.ts` entries. Three test files deleted; `PropertyControlRenderer.test.tsx`'s affordance-eligibility suite replaced with one that pins the new contract — **no row offers a binding control** — so the CMS-era affordance cannot quietly reappear.

**What was deliberately NOT removed, and why it is not a half-measure:** the ENGINE that renders bindings already stored in a page tree — `@core/page-tree`'s `dynamicBindings` field and schema, `@core/templates/dynamicBindings`, `@core/publisher`'s `dynamicDetection`/`renderNode`, `server/publish/*`, and the canvas preview path (`NodeRenderer`'s `resolveDynamicProps`). Three reasons, none of them "to keep the diff small": `dynamicBindings` is a **persisted** page-tree field on live installations (CLAUDE.md's DB/stored-JSON rule — a reader that no longer understands stored shapes is a data-loss bug, not a cleanup); it is one of the four rules in the publisher's request-dependency detection, which decides `<studio-hole>` emission for dynamic routes; and template/entry pages render through it. The result is coherent rather than dual-path: **Studio no longer offers data binding as an authoring feature, and the CMS publisher still honours bindings that already exist in stored content.** Removing the engine too is a real option, but it is a CMS-publisher change with a data migration, not a panel change — ask before starting it.

**Files:** `src/admin/pages/site/property-controls/{PropertyControlRenderer.tsx,controls.module.css,SlotPicker.tsx}`, `src/admin/pages/site/panels/PropertiesPanel/{renderModuleTabContent.tsx,PropertiesPanel.tsx,usePropertiesPanelData.ts,ParamPromotableRow.tsx}`, `src/admin/pages/site/store/slices/site/{nodeActions.ts,types.ts}`, `src/ui/components/ContextMenu/ContextMenu.tsx` (stale comment), tests `src/__tests__/property-controls/PropertyControlRenderer.test.tsx` + `src/__tests__/editor-store/noOpMutationContract.test.ts`; deleted `DynamicBindingControl/`, `bindingCompatibility.ts`, `src/__tests__/property-controls/{bindingCompatibility,dynamicBindingControl,dynamicBindingPicker}.test.*`.

### mcp-10 — cutting model round trips out of the studio verify loop: batched compare/quality-check/fidelity-report, a compare verdict cache, and the bridge retry moved server-side
- **Agent:** mcp-tooling
- **Stage:** implemented, tests added and green (`compare.test.ts`, new `compareVerdictCache.test.ts`, `qualityCheck.test.ts`, `fidelityReport.test.ts`, `editorBridge.test.ts`), `bun run build` + `bun run lint` clean on every touched file (two pre-existing `tsc`/lint failures in `designVariableStore.ts`/`referenceMeasure.ts` are a concurrent session's, confirmed via `git status`, not mine).
- **Updated:** 2026-08-30
- **Goal:** cut the model round trips a verify loop pays for — one call per screen where one call for N screens would do, a full recapture+diff for a page that provably hasn't changed, and a whole extra tool call just to retry a bridge connect the tool can retry itself.

**CHANGE A — plural `pages` on `studio_compare`/`studio_quality_check`/`studio_fidelity_report`.** All three took a single `page`/`pageId`; now all three (plus `studio_screenshot`, already plural) share ONE shape via a new `MAX_BATCH_PAGES = 20` export in `pageNameMatch.ts`: an optional, name-resolved, capped array. Per-page work loops; per-batch work (board sync, `loadStudioPages`, the shared ts-morph `Project`, the live-reload wait, `compileProjectStyles`/token index for quality-check) runs exactly once — mirrors `screenshot.ts`'s own `canonicalProject` split. One page failing to resolve/capture never fails the batch: it becomes a `results[]` entry with `ok:false` while the rest still return. `studio_compare`'s top-level `pass` is honest — `true` only when every requested name resolved AND every resolved page passed; an unmatched name or a per-page error always drags it to `false`, never a silent pass. `includeImages` on `studio_compare` defaults to `true` for exactly one resolved page (unchanged behaviour) and to `false` the moment a call resolves to more than one — three images/page was already the expensive part of one response, and the rule is stated explicitly in the tool description rather than left for the model to infer. `studio_fidelity_report` deliberately does NOT cap the "omit `pages`" case at 20 the way the other three do — it's a project-wide report, each entry is compact, and truncating a 30-screen project's report would hide exactly the gap the tool exists to surface; an EXPLICIT `pages` selection still shares the family's cap. No dual old/new param — every call site (tests, `resources.ts`'s guideline text) updated to the plural shape, nothing left accepting the old singular name.

**CHANGE B — `studio_compare` verdict cache** (`compareVerdictCache.ts`, new). Same mtime-tracking technique as `pageParseCache.ts` (read that file's doc first) but with one deliberate divergence: `fileMtimeOrZero` records `0` for a not-yet-existing file instead of omitting it, because `pageParseCache`'s own "just skip it" behavior would make a LATER creation of that file invisible to the validity check — unacceptable here since "a stale pass is far worse than a slow compare." Tracked per page: its own source file, every stylesheet `collectPageStylesheets` finds it importing (same one-level scope `studio_quality_check` already accepts), `.studio/framework.json` (the token store), and `.studio/boards.json` (a resized frame changes the capture's dpr/pixel size without touching the page's own files). `passScore`/`maxRegionCoverage`/`topN`/the resolved reference's own `id` are folded into the CACHE KEY, not the mtime set — they change what the SAME pixels mean, so a differing threshold is a different entry, never a stale reinterpretation. A cache HIT never touches the bridge at all: `compare.ts` now resolves every page's reference + cache key BEFORE deciding whether it needs the bridge, so an all-hit batch connects to nothing (verified live: the test `never touches the bridge at all when every requested page is a cache hit` sets `bridgeImpl = null` — simulating a closed board — and the cached call still succeeds). A MISS pays the stylesheet-discovery parse (one ts-morph parse, shared `Project` per batch) ONLY alongside the much larger capture+diff cost it was already paying. `forceRecapture` is the explicit escape hatch (documented limitation: a project-wide Tailwind/PostCSS/Sass toolchain change is not tracked — recomputing that on every cache CHECK would cost as much as the capture the cache exists to avoid). Every result carries `fromCache`.

**CHANGE C — the bridge retry moved from the system prompt into `editorBridge.ts`.** `awaitEditorBridgeForUser` already waited one `BRIDGE_WAIT_MS` (~4s) window; the system prompt (`server/ai/tools/studio/systemPrompt.ts:244`, owned by a concurrent session — NOT edited here) told the model "if [studio_screenshot/studio_compare] reports no connected board, CALL IT ONCE MORE." That second wait window now lives INSIDE the function (`BRIDGE_MAX_ATTEMPTS = 2`, same total patience the old two-call sequence bought, now inside one call) — `signal.aborted` is checked before every poll in every window, so a cancelled turn still returns immediately rather than sitting through the second window (pinned by a new test: an already-fired `AbortSignal.timeout(50)` returns in well under 2s, not ~8s). `server.ts`'s generic browser-tool relay and `liveReloadPush.ts`'s fire-and-forget push both call `getEditorBridgeForUser`/`hasEditorBridge` directly, never `awaitEditorBridgeForUser` — their fast-fail behavior is untouched by this change, confirmed by grep before editing.

**Prompt edit needed (I do not own `systemPrompt.ts` — reporting, not applying):** in the paragraph starting "TREATING A DISCONNECTED BOARD AS A DEAD END" (`systemPrompt.ts:244`), the sentence `"The tab reconnects by itself within a few seconds of any interruption, and both tools already wait for it — so if one reports no connected board, CALL IT ONCE MORE before concluding anything. Only if it fails twice is the project genuinely not open; say so in one sentence then, and do not write a pile of files you have no way to verify."` is now FALSE — the tool itself now spans two wait windows before answering, so a "no connected board" response already reflects ~8s of internal retry and a second call would just repeat the same wait for nothing. Replace with something to the effect of: `"The tab reconnects by itself within a few seconds of any interruption, and the tool already waits through two full reconnect windows internally before answering — so if it STILL reports no connected board after that, the project is genuinely not open. Say so in one sentence and do not write a pile of files you have no way to verify; do not call it again expecting a different answer with nothing else having changed."`

**Files:** `server/ai/mcp/tools/studio/{compare.ts,qualityCheck.ts,fidelityReport.ts,pageNameMatch.ts,screenshot.ts}`, new `server/ai/mcp/tools/studio/compareVerdictCache.ts`, `server/ai/mcp/editorBridge.ts`, `server/ai/mcp/resources.ts` (one stale `pageId?` doc reference), tests `server/ai/mcp/tools/studio/{compare.test.ts (rewritten),qualityCheck.test.ts,fidelityReport.test.ts}` + new `compareVerdictCache.test.ts`, `server/ai/mcp/editorBridge.test.ts` (extended). **Not touched, per the task's explicit boundary:** `server/ai/tools/studio/{systemPrompt.ts,liveDigest.ts}`, `server/ai/mcp/tools/studio/typecheck.ts`, `server/handlers/studio/typecheck.ts`.

**Not done / next:** the prompt edit above is the only remaining piece of this work order — apply it, then this loop is fully closed. `studio_diff_frames`/`studio_export_frames` were left as-is (external-client-facing, no batching gap the task named).

### mcp-tooling-design-variables — measure against the design's OWN declared values, not just pixels
- **Agent:** mcp-tooling
- **Stage:** implemented, tests added and green, `bun run build` + `bun run lint` clean.
- **Updated:** 2026-08-30
- **Goal:** `studio_measure_reference` inferred colour/type-size purely from pixels. Figma's `get_variable_defs` (reachable by the AGENT, never by Studio's server — see architecture note below) already states those values exactly. Nothing ingested that table. This adds the ingestion boundary and wires a three-way lookup — measured pixel → declared design variable → project token — into the existing tool, additively.

**Architecture: Studio's server never talks to Figma.** The Figma connector, when configured, is available to the `claude` subprocess (the in-canvas agent), gated behind its own three conditions — never to this server. So the shape is: the agent calls the design tool itself and hands the resulting table to `studio_ingest_design_variables`, which stores it as **what the agent was given, never something Studio independently verified**. Every tool description and the store's own module doc say this explicitly.

**Storage** (new, mirrors `designReferenceStore.ts`'s conventions exactly): `server/handlers/studio/{designVariableSchema.ts,designVariableStore.ts}`. Persisted at `.studio/variables/manifest.json` (new `.gitignore` entry, same durability class as `.studio/references/` — survives server restarts, not a git clone). A "set" is one ingest call's worth of name/value entries, associated with the **project always**, and optionally with one `pageId` and/or one `referenceId` — never forced onto either, because a real Figma variable table usually describes a whole file, not one screen. `resolveApplicableDesignVariableSets(dir, pageId, referenceId)` returns project-wide ∪ page-scoped ∪ reference-scoped sets (a set scoped to a *different* page/reference never applies — exclusionary, not a suggestion). Duplicate names **within one ingest call** collapse (last wins, `duplicatesDropped` reported); duplicate names **across sets** do not dedupe — each set stays independently addressable/removable.

**Normalisation** (new, pure, `designVariableNormalize.ts`): colours via the same `cssColorToRgb`/`rgbToHex` `projectTokenIndex.ts` already uses (hex/rgb()/hsl() all recognised as the same colour — `#EF4550` ≡ `rgb(239,69,80)`). Sizes: an explicit `px`/`rem`/`em`/`pt` suffix converts exactly (unit is knowable); a **bare number** is treated as px (Figma's own FLOAT convention for most geometry variables) but flagged `unitAssumed:true`, because a bare number could just as easily be an opacity, a line-height multiplier, or a font-weight — the ambiguity is real and this module never claims more certainty than the input supports. Anything else normalises to `kind:"other"` with `raw` preserved verbatim, never discarded, never thrown. Bounded against hostile input: `MAX_VARIABLES_PER_INGEST=1000` (`maxItems` on the ingest schema), name/value/source/label length caps, a `value` union of string/number/boolean only (no nested objects/arrays), and a plausibility ceiling (`MAX_PLAUSIBLE_PX=100_000`) so a garbage numeric string can't corrupt downstream nearest-match arithmetic. **A real gap found while testing the TypeBox boundary itself:** `Value.Parse`'s pipeline is Default→Convert→**Clean**→Decode→Check — Clean strips unknown properties (so `additionalProperties:false` silently drops extras rather than throwing) and Convert coerces across a `Union`'s branches (so `value: 1e300`, which fails the Number branch's `maximum`, silently becomes the STRING `"1e+300"` rather than erroring) — both are pre-existing `parseValue` behaviour, not specific to this tool, but worth knowing for the next MCP tool boundary test: assert what a hostile payload becomes, not that it throws.

**The three-way mapping** (`server/handlers/studio/{designVariableIndex.ts,referenceMeasure.ts}` — I own both, neither is on the forbidden list): `referenceMeasure.ts`'s `measureReference()` gained an optional `designVariableSets` option. When present, every measured `MeasuredColor` (background/foreground/palette) and the `fontSizePx` range gain an optional `designVariable` field: the nearest declared variable (name, authored `value`, normalised `hex`/`px`, ΔE/Δpx, `source`, `setId`) — and, resolved from **that variable's own declared value, not the noisy measured pixel**, the project token it maps to (`projectToken`). Absent entirely — never a guess — when no set is applicable or nothing is close enough; `MeasureReferenceResult.designVariableIndex.setCount` is the one field a caller reads to know whether any table was even consulted (`0` is the honest "measure by pixel alone" signal, unchanged from before this shipped). `measureReference.ts` (the MCP tool) resolves applicable sets via `resolveApplicableDesignVariableSets(dir, match.id, reference.id)` and threads them through; its description now tells the agent to prefer `designVariable` over the pixel-only `token`/`fontSizePx` range whenever both are present.

**Four new tools**, `server/ai/mcp/tools/studio/designVariableTools.ts`, registered in `server/ai/mcp/tools/studio/index.ts` (edited per the task's explicit allowance) and (ingest/list/read, not delete — same curation `studio_delete_design_reference` already gets) in `server/ai/tools/studio/agentToolNames.ts` (also explicitly allowed, re-read fresh immediately before editing per instruction):
- `studio_ingest_design_variables({ dir?, source, pageId?, referenceId?, label?, variables })` — `execution:'server'`, `mutates:true`, `studio.write`. `variables` is `{name, value, figmaType?, collection?, mode?}[]`, `value` a string/number/boolean union (accepts a FLOAT variable forwarded as raw JSON, not pre-stringified). Refuses an unknown `referenceId` with a message naming `studio_register_design_reference`. Plain-object return (no envelope wrapping — see below).
- `studio_list_design_variables({ dir?, pageId?, referenceId?, limit? })` — summaries only (id/source/counts), never the full variable arrays; capped+truncation-honest like every other list tool in this family.
- `studio_read_design_variable_set({ dir?, setId, nameContains?, limit? })` — the actual entries for one set, capped (default 200, max 500), substring-filterable.
- `studio_delete_design_variable_set({ dir?, setId })` — idempotent.

**A return-shape bug caught by my own tests, fixed before it shipped:** I initially wrapped the ingest/read handlers' return in `aiToolOk(...)`, which — per `toolOutput.ts` — produces `{ ok: true, data: <payload> }`. Since `list`/`delete` in the same family return their payload **flat** (matching `designReferenceTools.ts`'s own mixed convention — plain object unless the tool needs to attach an `images` block, which none of mine do), the family was internally inconsistent and my own handler-level tests caught it immediately (`result.set` was `undefined`). Fixed by returning plain objects from all four handlers; `aiToolError` stays for the two error paths (its `{ok:false,error}` shape is already a valid envelope either way).

**Honesty about absence, the load-bearing property:** three explicit tests pin it — (1) `referenceMeasure.test.ts`'s "honesty: with no design-variable sets, every designVariable field is absent and the index reports zero," (2) `studio_measure_reference`'s description states `designVariablesIndexed.setCount: 0` means "measure by pixel alone, exactly as before this existed," (3) every store read (`listDesignVariableSets`, `getDesignVariableSet`) degrades a corrupted/missing manifest to empty via `parseJsonWithFallback`, never a thrown error mid-turn.

**Tests:** `designVariableNormalize.test.ts` (colour-form equivalence, unit-knowability, hostile: absurd magnitude, 5000-char garbage, scientific notation, `NaN`/`Infinity` spelled out, malformed hex/rgb), `designVariableStore.test.ts` (ingest/list/read/remove round trip, duplicate collapsing, cross-set independence, scoping resolution, corrupted-manifest degradation), `designVariableTools.test.ts` (tool shapes, the TypeBox boundary against malformed/hostile input including the Clean/Convert findings above, end-to-end handler coverage including a real registered design reference), `referenceMeasure.test.ts` (extended: the three-way mapping for colour — including the "design variable matches but the project has no covering token, and says so" case — and for a `unitAssumed` size, plus the honesty-when-absent case).

**Files:** new `server/handlers/studio/{designVariableSchema.ts,designVariableNormalize.ts,designVariableStore.ts,designVariableIndex.ts}` + their `.test.ts` siblings, new `server/ai/mcp/tools/studio/designVariableTools.ts` + `.test.ts`; edited `server/handlers/studio/referenceMeasure.ts` (+ extended `.test.ts`), `server/ai/mcp/tools/studio/measureReference.ts`, `server/ai/mcp/tools/studio/index.ts`, `server/ai/tools/studio/agentToolNames.ts`, `.gitignore`.

**Not touched, per the task's explicit boundary:** `server/ai/mcp/tools/studio/{compare.ts,qualityCheck.ts,fidelityReport.ts,screenshot.ts,referenceResolve.ts}`, `server/ai/mcp/editorBridge.ts`, `server/handlers/studio/pageParseCache.ts`, `server/ai/tools/studio/systemPrompt.ts`, `liveDigest.ts`.

**Not done / next:** `systemPrompt.ts` (owned elsewhere) could usefully mention `studio_ingest_design_variables` alongside the existing `studio_measure_reference` guidance — not applied here since that file is off-limits for this task. `studio_compare`/`studio_quality_check` do not yet consult design variables (only `studio_measure_reference` does) — a natural follow-on once a work order owns those files.

### board-13 — you could fill an icon slot but never change your mind: there was no replace write in the system
- **Agent:** coordinator (direct)
- **Stage:** implemented, `tsc -b` + `bun run lint` clean, 6 new codemod tests. **Not dogfooded in a browser.**
- **Updated:** 2026-08-30

`SlotControl`'s own doc called Replace/Clear a deliberate omission, and for Clear it still is. For Replace it was simply a missing write: `insertJsxIntoSlotProp` only ever APPENDED, so picking a second icon wrapped both in a fragment (`icon={<><A/><B/></>}`) and the component rendered the two of them. "Choose a different icon" was not expressible anywhere in the product.

`insertJsxIntoSlotProp` now takes `mode: 'append' | 'replace'` (default `'append'`, so every existing caller is byte-identical). Replace overwrites the attribute's JSX value, and is refused on exactly the shapes append is refused on — an identifier, a call, a ternary — for the identical reason: Studio cannot tell a real binding from "nothing", and overwriting one would destroy it silently. `propName: 'children'` refuses with a new `replace-not-supported`, because the default slot holds ordinary child NODES with their own ids and delete path, and silently appending there would be the wrong write.

**The part worth knowing about: replace prunes the import the old value was the last user of.** Swapping `<ChevronRightIcon/>` for another glyph otherwise leaves it imported and unused, which trips `noUnusedLocals` — Studio would hand back a file that no longer builds. `deleteJsxElement` REFUSES in the same situation (`orphans-import`, "remove the element and its import together") and that stays right for a structural delete, where the user asked to remove one thing and the codemod has no mandate to touch another. A slot replace is the opposite case: the user asked to SWAP this value, so retiring the binding it alone was using is part of the requested write. It is checked, never assumed — only a name with zero remaining non-import references is removed, and a declaration is only deleted once emptied by that pruning, so a side-effect `import './styles.css'` is untouched. Pinned by a test where a second call site still uses the old icon and the import correctly survives.

UI: a filled slot now shows **Edit contents · Replace · Add another**. `SlotControl` holds `pickerMode: 'append' | 'replace' | null` — one piece of state instead of an `open` flag plus a mode that can disagree with it.

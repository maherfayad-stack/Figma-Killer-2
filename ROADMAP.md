...
All sixteen program decisions are settled (OD-15 and OD-16 added by the owner on 2026-09-24). Their full text, with how each was answered, is in
...
| OD-16 | Bulk actions on a multi-selection: every action that makes sense on many layers acts on all of them as one gesture and one undo (owner, 2026-09-24) | P2-C2 |
...
| **P1-H** User projects survive a container recreate | found by P0 (docs-15) | The shipped Docker images, compose files and deploy templates keep `studio-workspace/` outside every volume, so a user's projects are **lost** when the container is recreated. Put `studio-workspace/` on a persistent volume in every compose file and template (`docker-compose.yml`, `compose.*.yml`, `Dockerfile`, the Railway/Render/VPS docs), migrate an existing container's workspace safely on upgrade, and cover it with a test that inspects the compose/Dockerfile config. Also fix the `trustGate.ts` comment that still says Tier 0 is the default | server-engineer (+ security-guard review) | S–M |

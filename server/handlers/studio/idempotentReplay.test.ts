...
describe('withIdempotentReplay', () => {
  ...
  it('replays the stored response for a key already seen, WITHOUT re-running the handler', async () => {
    ...
    const first = await withIdempotentReplay(requestWithKey(key), run, root)
    expect(await first.json()).toEqual({ ok: true, effect: 1 })

    const replay = await withIdempotentReplay(requestWithKey(key), run, root)
  ...
  it('does not cache a non-2xx response — a genuine refusal is safe to recompute', async () => {
    ...
    await withIdempotentReplay(requestWithKey(key), run, root)
    await withIdempotentReplay(requestWithKey(key), run, root)
    expect(calls).toBe(2)
  })
  ...
  it('two different keys never collide', async () => {
    ...
    const resA = await withIdempotentReplay(requestWithKey(keyA), async () => Response.json({ who: 'a' }), root)
    const resB = await withIdempotentReplay(requestWithKey(keyB), async () => Response.json({ who: 'b' }), root)
  ...
  it('the record survives being read by a brand-new process — i.e. is not in-memory', async () => {
    ...
    const key = crypto.randomUUID()
    await withIdempotentReplay(requestWithKey(key), async () => Response.json({ ok: true }), root)
  ...
  it('an expired record is not replayed — recomputes instead', async () => {
    ...
    const second = await withIdempotentReplay(requestWithKey(key), run, root)
    expect(calls).toBe(2)

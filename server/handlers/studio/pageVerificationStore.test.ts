...
describe('pageVerificationStore', () => {
  ...
  it('records and reads back a passing compare', () => {
    recordPassingCompare(dir, USER, 'onboarding', 'ref-1', 12345)
    expect(readPassingCompare(dir, USER, 'onboarding')).toEqual({ passedAtMs: 12345, referenceId: 'ref-1' })
  })

  it('records for one page without disturbing another', () => {
    recordPassingCompare(dir, USER, 'onboarding', 'ref-1', 1000)
    recordPassingCompare(dir, USER, 'checkout', 'ref-2', 2000)
    expect(readPassingCompare(dir, USER, 'onboarding')).toEqual({ passedAtMs: 1000, referenceId: 'ref-1' })
    expect(readPassingCompare(dir, USER, 'checkout')).toEqual({ passedAtMs: 2000, referenceId: 'ref-2' })
  })

  it('a later record for the same page overwrites the earlier one', () => {
    recordPassingCompare(dir, USER, 'onboarding', 'ref-1', 1000)
    recordPassingCompare(dir, USER, 'onboarding', 'ref-2', 2000)
    expect(readPassingCompare(dir, USER, 'onboarding')).toEqual({ passedAtMs: 2000, referenceId: 'ref-2' })
  })

  it("one account's passing compare does not satisfy another's gate", () => {
    recordPassingCompare(dir, OTHER_USER, 'onboarding', 'ref-1', 1000)

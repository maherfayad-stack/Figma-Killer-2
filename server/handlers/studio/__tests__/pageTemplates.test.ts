...
describe('detectPageTemplateKit', () => {
  ...
  /**
   * The kit is decided by the FOLDER, never by a dependency. There is no
   * design-system package any more: an unmigrated project that still declares
   * the retired npm must NOT be scaffolded with an import of it, because that
   * import no longer resolves to anything.
   */
  ...
  it('picks the plain kit for a bare folder of .tsx files, with no package.json at all', () => {

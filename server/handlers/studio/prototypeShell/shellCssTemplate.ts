/**
 * shellCssTemplate — `shell.css`: the exported prototype's own chrome.
 *
 * Split out of `shellFiles.ts` when the mobile settings sheet pushed that file
 * past the 700-line ceiling, and split HERE because this is the seam the
 * directory already uses: `canvasTemplate`, `screenFrameTemplate` and
 * `playerTemplate` are each one emitted file, and this is the fourth.
 *
 * Everything in here styles the TOOL — the bar, the option rows, the settings
 * sheet, the device shell. Nothing in here reaches inside a frame: that is the
 * project's own CSS, mirrored into each iframe by `ScreenFrame`, and the two
 * must never be confused. The class prefix is `shell__` for exactly that
 * reason.
 *
 * Same no-backticks rule as every template here: this is a template literal.
 */

export const SHELL_CSS = `/* The shell's own chrome. Everything inside a frame is the project's CSS —

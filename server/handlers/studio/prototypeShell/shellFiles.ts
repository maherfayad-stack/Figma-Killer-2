 ...
 *
 * A file here is rewritten only while it is still byte-for-byte the one Studio
 * last wrote (`.studio/shell.json` records the hash — see `index.ts`). Edit
 * `App.jsx` and Studio will never touch it again; leave it alone and a fix to
 * this template reaches your workspace on the next open. The two files Studio
 * always keeps in step end in `.generated.jsx` and say so at the top — see
 * `registryFile.ts`.
 *

...
import { isUnlinkedWorkspacePath, pathEntryExists } from '@core/page-parser'
 ...
 *
 * ## Only what Studio wrote (security review of #233, F7)
 *
 * These are generic names — `.claude/agents/design-critic.md` is a name a
 * repository's own authors could have chosen — so a name alone never proves
 * Studio wrote the file. {@link pruneLegacyGuideArtefacts} deletes one only
 * when Studio's own manifest says Studio has generated a guide in this project
 * before (an imported repository's first turn has none, so its `.claude/` is
 * left exactly as it arrived), and, where the manifest still carries the
 * file's hash (a project last generated by the roster itself), only when the
 * file is byte-identical to what Studio wrote. Where the hash was already
 * dropped (an intermediate generator stopped recording it), the name inside a
 * Studio-generated `.claude/` is the evidence, and a hand-edited copy
 * describing a removed subsystem still describes a removed subsystem. Never
 * through a link: the file and every directory above it must be plain
 * entries inside the project (`isUnlinkedWorkspacePath`). The
 * once-per-project record above keeps the sweep from becoming a recurring
 * delete.
 */
/**
...
export function pruneLegacyGuideArtefacts(dir: string, manifest: GeneratedManifest): PruneLegacyArtefactsResult {
  ...
  // No manifest on disk: Studio has never generated a guide here, so nothing
  // under these names is Studio's. Marked swept, deleted never.
  const studioGeneratedHere = pathEntryExists(join(dir, MANIFEST_PATH))
  ...
  for (const rel of outstanding) {
    ...
    try {
      ...
      // never had.
      if (studioGeneratedHere && isStudioLegacyArtefact(dir, abs, manifest.files[rel]?.hash)) {
...
/** Whether `abs` is a plain file Studio may treat as its own legacy output — see {@link LEGACY_GUIDE_ARTEFACTS}. */
function isStudioLegacyArtefact(dir: string, abs: string, recordedHash: string | undefined): boolean {
  if (!pathEntryExists(abs) || !lstatSync(abs).isFile()) return false
  if (!isUnlinkedWorkspacePath(dir, abs)) return false
  return recordedHash === undefined || sha256(readFileSync(abs, 'utf8')) === recordedHash
}
...
export function writeManifest(dir: string, manifest: GeneratedManifest): void {
  const path = join(dir, MANIFEST_PATH)
  // Never through a link: a `.claude` junction out of the project would
  // carry this write — and every guide file — somewhere else.
  if (!isUnlinkedWorkspacePath(dir, path)) return

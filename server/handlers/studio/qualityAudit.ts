 ...
 * things a token swap cannot — coverage here, composition in
 * `compositionAudit.ts`:
 *
 ...
 * `studio_quality_check` response. See that module's doc for the rules
 * themselves; it reuses this module's scan primitives rather than restating
 * them.
 *
...
/** A bare literal px length. Exported for `compositionAudit.ts`, which reads the same authored values at a different unit of aggregation. */
export const RAW_PX_RE = /^(-?\d+(?:\.\d+)?)px$/
/** A rule block: `selector { declarations }`, non-nested (matches `single-styling-mechanism`'s textual-scan posture — no real CSS parser). Exported so `compositionAudit.ts` scans the SAME text the same way — two textual scans that disagreed about what a rule is would grade the same stylesheet differently. */
export const RULE_BLOCK_RE = /([^{}]+)\{([^{}]*)\}/g
/** One `property: value` declaration inside an already-captured rule block. Exported alongside `RULE_BLOCK_RE`, for the same reason. */
export const DECLARATION_RE = /([a-zA-Z-]+)\s*:\s*([^;]+);?/g
...
/** 1-based line of `index` in `text`. Exported for `compositionAudit.ts`, so a finding's `line` means the same thing in both audits. */

...
/**
 * DELETE /admin/api/studio/page — removes a page from the project for real:
 * its source file, an orphaned stylesheet, its board frames. Driven against a
 * temp `dir`, like its POST sibling, so the repo's real studio-workspace is
 * never the thing being deleted.
 */
describe('DELETE /admin/api/studio/page', () => {
  let tmpDir: string

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'studio-delpage-'))
  })
  ...
  it('deletes a scaffolded page end to end — file, stylesheet and board frame', async () => {
    const created = await send('POST', { dir: tmpDir, name: 'Contact Us' })
    const { pageId, relPath } = (await created.json()) as { pageId: string; relPath: string }
    const pageFile = path.join(tmpDir, 'pages', relPath)
    expect(fs.existsSync(pageFile)).toBe(true)

    const res = await send('DELETE', { dir: tmpDir, pageId })

    expect(res.status).toBe(200)
    const body = (await res.json()) as { ok: boolean; removedFiles: string[]; removedFrames: number }
    expect(body.ok).toBe(true)
    expect(body.removedFiles).toContain(`pages/${relPath}`)
    expect(body.removedFrames).toBe(1)
    ...
    expect(boards.boards.flatMap((board) => board.frames)).toEqual([])
  })

  it('answers 404 for an id no page has, without touching the project', async () => {
    await send('POST', { dir: tmpDir, name: 'Home' })

    const res = await send('DELETE', { dir: tmpDir, pageId: 'not-a-page' })

    expect(res.status).toBe(404)
    expect(fs.existsSync(path.join(tmpDir, 'pages', 'Home.tsx'))).toBe(true)
  })

  it('rejects a body with no pageId', async () => {
    const res = await send('DELETE', { dir: tmpDir })

 ...
 *
 * `./handlers/studio/devServer` (L1) and `./handlers/studioProjects`'s
 * `resolveExistingProjectDir` are replaced here with `mock.module`, using
 * the EXACT relative specifiers `liveOrigin.ts` itself imports (same
 * technique `compare.test.ts` documents), backed by one plain in-memory
 * registry this file controls per test, keyed by the SAME `dir` string
 * `resolveProjectDirForKey` computes at runtime
 * (`join(projectsRootDir(), projectKey)`, real `node:path.join`, not
 * mocked) — an unregistered `projectKey` resolves to a `dir` that just
 * isn't in the registry, so the mocked `resolveExistingProjectDir` reports
 * it as "no such project" (`null`), the real "unknown project" 404 path.
 * The containment/existence logic itself is `resolveExistingProjectDir`'s
...
mock.module('./handlers/studio/devServer', () => ({
  ...
  getDevServerUpstreamUrl: (dir: string): string | null =>
    registry[dir]?.status.phase === 'ready' ? (registry[dir]?.url ?? null) : null,
}))
...
function registerProject(projectKey: string, phase: DevServerStatus['phase'], url = UPSTREAM_ORIGIN): void {
  // Keyed by the SAME `dir` string `resolveProjectDirForKey` computes at

 ...
 */
/**
 * The "a disconnected board is not a dead end" paragraph, GENERATED from each
 * offered tool's `execution` metadata rather than written by hand.
 *
 * ## Why it is generated
 *
 ...
 * does not produce an error, it produces a tool that never gets called.
 *
 * Both lists now come from the same `execution` field the RUNNER dispatches
 * on (`runtime/toolExecution.ts`), off the caller's own capability-filtered

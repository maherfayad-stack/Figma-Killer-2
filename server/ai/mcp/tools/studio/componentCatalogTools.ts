...
function emptyCatalogNote(
  ...
  if (installedPackages.length > 0) {
    ...
    return `${names} IS installed, but ships no .d.ts/.tsx type declarations, so no component/prop list can be extracted from it — an empty catalog here does NOT mean the components are missing. Its exports are real and importable: write them as an ordinary import (e.g. import { Button } from "${installedPackages[0]}"). To learn the available names and props, read the package's own docs (its README under node_modules), use its own MCP server if the project has approved one, or read .claude/design-system.md for the class/token index.${imported.length > 0 ? ` This project also carries a CSS copy: ${importedNames}.` : ''}`
  }

  if (imported.length === 0) return undefined
  return `This project has no installed component package to read a component API from, but it does have a CSS-only imported design system: ${importedNames}. That copy is plain CSS — no .d.ts/.tsx, so no component/prop list can be extracted from it. Its stylesheets (one .css file per component name under its root) and design tokens are still real and readable — read the files directly, and studio_list_tokens lists the tokens. Build with intrinsic HTML tags carrying these classes; do NOT write an import for a package this project does not have.`
}
...
const listComponentsTool: AiTool = {
  ...
  sideEffects: 'none',
  description:
    'List the project\'s design-system components (the insert palette\'s catalog, plus Code Connect-only ones). Entry: { name, pkg, exportName, isDefaultExport, file, hiddenFromPalette, apiSource, props[{ name, kind, required }], description?, keywords?, group?, figma? }. Import from pkg (a default import when isDefaultExport), except apiSource \'builtin\', where pkg is the project\'s design-system/ folder and the import is relative to the importing file. apiSource \'types\': required is reliable; \'code-connect\': required is always false. hiddenFromPalette marks an overlay (Dialog, Toast…) you compose directly. filter (name substring) and package narrow it; capped (default 60, max 200) with omittedCount. A CSS-only imported design system has no component API: see designSystems and note. Knowing a name or prop? Use studio_find_component.',
...
const findComponentTool: AiTool = {
  ...
  sideEffects: 'none',
  description:
    'Search the design-system catalog studio_list_components lists, by name and/or prop: name substring-matches the component, prop substring-matches a declared prop name (prop:\'variant\' finds every component with one, so their enum values can be compared), and both together narrow further. At least one is required; to browse, use studio_list_components. Returns the same entry shape ({ name, pkg, exportName, isDefaultExport, file, hiddenFromPalette, apiSource, props, figma? }), capped (default 40, max 200) with truncated and omittedCount. A CSS-only imported design system has no component API and never matches: check designSystems and note to tell \'nothing matched\' from \'nothing to search\'.',

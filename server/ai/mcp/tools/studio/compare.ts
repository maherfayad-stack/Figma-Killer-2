...
export const studioCompareTool: AiTool = {
  ...
  description:
    'Measure one or more screens against their registered design reference and get a verdict: { pass, results[] }. Name screens by file ("Checkout"), several for a flow. Captures headless on the server from disk (an open tab only as fallback; capturedVia says which). Each result has pass, similarityScore and regions[] — rectangles that are actually wrong, worst first, with the node ids inside and, on a failing page, colorExplanation naming each side\'s colour in project tokens — plus your screen, the reference and the diff as images (single page). pass is not pixel identity. A page with no reference or a failed capture is ok:false and fails the top-level pass. dimensionMatch \'resampled\' is directional; \'cropped-to-reference\' measured only the top comparedHeight band, the rest is UNMEASURED. Unchanged pages come from cache (fromCache) unless forceRecapture. Fix the largest region first.',
  ...
  handler: async (input, ctx: ToolContext) => {
    ...
    if (ids.length === 0) {
      ...
      return unmatched.length > 0
        ...
        : toolRefusal('no-such-page', 'This project has no screens to compare yet.', {
            remedy: 'Write the page file first, then measure it.',
          })
    }
    ...
    const projectColorTokens = (): Promise<RegionExplainVocabulary['projectTokens']> => {
      projectTokensPromise ??= (async () => {
        try {
          return buildProjectTokenIndex(...(await collectProjectTokenCss(dir)))
        } catch (err) {

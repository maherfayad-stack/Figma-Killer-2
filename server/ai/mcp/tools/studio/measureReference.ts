...
export const studioMeasureReferenceTool: AiTool = {
  ...
  sideEffects: 'none',
  description:
    'Read the design\'s ACTUAL colours and type sizes from a registered reference instead of guessing them from the picture. Give the screen name and rectangles in the reference image\'s own pixels. Per region: background and foreground hex with the nearest project token, WCAG contrast, the dominant palette, text lines, a font-size RANGE (fontSizePx.caveat: assumes a Latin sans, coarse for serif, display or non-Latin text) and the exact line-height, all in CSS px for the screen\'s frame width. Use it BEFORE writing a stylesheet for a screen with a design: choosing a token by its name skews large. When no token is in range it says so, and a raw value is the honest choice. After studio_ingest_design_variables, results also carry designVariable (the design\'s own declared value and its token): prefer it. designVariablesIndexed.setCount 0 means pixels only.',

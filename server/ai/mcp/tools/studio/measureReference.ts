...
export const studioMeasureReferenceTool: AiTool = {
  ...
  handler: async (input, ctx: ToolContext) => {
    ...
    const result = await measureReference(bytes, regions, { cssScale, cssSources })
    ...
    return aiToolOk({
      ...
      units: 'All lengths are CSS px, already scaled from reference pixels. fontSizePx is a range: capAssumption is the lower bound (the measured ink is cap height, no descender), ascenderAssumption the upper (the ink spans ascender to descender). lineHeightPx is measured from line pitch and needs no assumption.',

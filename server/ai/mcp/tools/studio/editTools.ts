...
const applyEditsTool: AiTool = {
  ...
  description:
    'Apply a batch of typed source edits to a project\'s .tsx/.jsx files and stylesheets in one call. Value kinds rewrite one span in place: prop, text, style, class, literal, tag, asset. Structural kinds: insert (nodeId is the CONTAINER; children nests a whole subtree, siblings adds a run after it — build a screen in ONE insert, never one element per call), delete, move, duplicate, wrap, group, ungroup, transplant, styled, reinsert-source, detach, swap. css writes a declaration into a hand-authored stylesheet, creating the rule when missing. Returns { written, skipped, shifted, sharedComponents, refusals, fingerprints, retargeted, pageIds }: shifted:true means node ids read before this call are stale. refusals name each reason (binding-overwrite, element-moved, css-module-binding, template-dynamic, spread-attribute, unsupported-expression, …). Rules per kind: MCP resource studio://tool-notes. Requires studio.write.',

...
const applyEditsTool: AiTool = {
  ...
  description:
    'Apply a batch of typed source edits to a project\'s .tsx/.jsx files in one call — the same engine POST /admin/api/studio/save runs (ordering bottom-to-top so a line-shifting codemod can\'t invalidate a pending edit\'s location, dedup, per-edit try/catch, shared-component detection). Seven VALUE kinds rewrite a single existing span in place: prop, text, style, class, literal, tag, asset. They do NOT reliably preserve line count — a text edit can re-flow children onto several lines and a style edit can rewrite a multi-line style={{…}} object — so read shifted after every call. prop only ever replaces a LITERAL attribute value (or adds a missing attribute): an attribute that holds code (title={c.heading}, onClick={fn}) refuses with reason binding-overwrite rather than baking a literal over the binding. detach and swap also rewrite an existing element, but by inlining or retargeting a WHOLE component body — typically many lines, so treat them as line-count-changing too. Three STRUCTURAL kinds change WHERE markup is and always change the file\'s line count: insert (add a new element — nodeId is the CONTAINER\'s location, not the new element\'s; optional anchorNodeId/position places it beside an existing sibling, default appends as the last child. WITH importSpecifier, name is a component to import from that exact specifier, e.g. "Button" from "@acme/ui"; WITH designSystemImport:true (and no importSpecifier), name is a component of Studio\'s BUILT-IN design system — the server computes the relative path to the project\'s own design-system/ folder for the file being written, so never write that path yourself; WITHOUT either, name is a plain intrinsic HTML tag — "div", "span", "button", "img" — which needs no import, and is how you build the layout structure a screen is made of. children builds the element\'s CONTENT in the SAME call: either a literal text string (<span>Sign in</span>) or an ARRAY OF NESTED ELEMENTS, each with the same { name, importSpecifier?, designSystemImport?, props?, children? } shape, nested arbitrarily deep. BUILD A WHOLE SCREEN IN ONE insert — do NOT insert one element per call and re-read between them: every insert shifts node ids, so per-element inserts cost a re-parse each and a ~30-node screen becomes ~30 sequential round trips (measured: over twenty minutes for one screen). A nested subtree needs no intermediate node ids at all, because nothing reads one between the levels. All imports for the whole tree are written in one pass, and the whole subtree is validated before any byte is written, so a bad grandchild refuses without leaving a half-built element. siblings — an array of more elements in the same node shape — writes a RUN of new elements right after this one, at the same anchor, in the same write; createdNodeIds then names every one in order), delete (remove an element — the response\'s removed/prunedImports carry exactly what it took out), move (reorder — nodeId is the element being moved, anchorNodeId + position name where it goes). The batch also accepts duplicate, wrap, group, ungroup, transplant, styled, and reinsert-source (a delete\'s own undo: nodeId is the PARENT, index the child position, text the bytes a previous delete returned in removed — text must be JSX content and nothing else, a statement or an unmatched closing tag is refused by name). The class kind ({ kind: "class", nodeId, add: string[], remove: string[] }) adds/removes whole class TOKENS in an element\'s className attribute — add/remove are class NAMES (e.g. "bg-blue-600"), never the editor\'s own "sc-<hash>" style-rule ids. This is how a Tailwind (or any plain-string-class) element edit reaches disk: swapping bg-red-500 for bg-blue-600 on an element is a class edit, not a css edit — css below only ever targets a hand-authored stylesheet rule. class writes a bare className="a b" literal, an expression-wrapped string/template (className={"a b"} or className={`a b`}), and cn(...)/clsx(...)/classNames(...)/classnames(...) calls (ADD merges into a literal argument or appends one; REMOVE strips the token from every literal argument, best-effort — a token reachable only through a non-literal argument, e.g. isActive && "active", is left alone); it creates the attribute when absent. It refuses by name rather than guessing: css-module-binding (className={styles.card}, a default import from a *.module.css file — edit the class\'s own declaration in the stylesheet instead), template-dynamic (removing a token from a dynamic template literal, e.g. `a ${x}` — a token might live in the interpolated part, which can\'t be read from source text; ADD to the same shape still works, appended to the static head), spread-attribute (className={...spread}), and — for a REMOVE only — unsupported-call / unsupported-expression: an ADD to a bare identifier, member chain, ternary, logical, or any other call is WRAPPED instead, keeping the binding (cn(expr, "a") when the file already has a class-join helper, else `a ${expr || \'\'}`); a className that is not a class string at all (an arrow, an object) still refuses unsupported-expression. A module token whose stylesheet the file does not import yet is written as styles.<local> and the import is added after the batch\'s last edit (shifted is then true). A request where every add token is already present and every remove token already absent is a silent no-op, not a refusal. One more kind, css, writes a declaration into a stylesheet directly (file + selector + property + value) rather than a JSX node — the rule is CREATED at the end of the file when the selector is not there yet, so this both edits existing styles and authors new ones (e.g. filling in the .module.css studio_create_page scaffolds next to each page). The file must already exist and be hand-authored CSS: a compiled/generated stylesheet is refused by name. This is the "make big changes at once" tool — for a single detach/swap, studio_codemod\'s richer per-call result is usually more convenient. Returns { written, skipped, shifted, sharedComponents, refusals, fingerprints, retargeted, pageIds } — `shifted: true` means a write changed a touched file\'s line count, so any node id you decoded BEFORE this call is now stale (re-call studio_list_pages/studio_find_nodes) — this is GUARANTEED for insert/delete/move, LIKELY for detach/swap, and POSSIBLE for text/style (never assume a value edit left ids intact — check this field); `sharedComponents: true` means an edit landed on an inlined component instance, route chrome, or a detach/swap; `refusals` lists WHY any edit specifically didn\'t write, with a named reason rather than a generic skip (element-moved when an expect entry no longer matches and the element cannot be found exactly once in the changed file: re-read the node ids and retry against the current ones); `fingerprints` gives each landed value edit\'s target its new sourceFingerprint, for the next call\'s expect; `retargeted` lists { nodeId, to } for every id whose file changed on disk since you read it but whose element (matched by its expect fingerprint through a line diff) was re-found exactly once and written at `to` — shifted is then true, so re-read before reusing ids in that file; `pageIds` names the pages this batch touched — if the caller has the project open in a browser tab, its canvas is nudged to re-read exactly those pages (best-effort; nothing to do if no browser is open). Requires studio.write.',

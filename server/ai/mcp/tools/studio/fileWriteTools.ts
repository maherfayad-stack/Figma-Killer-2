...
const writeFileTool: AiTool = {
  ...
  description:
    'Create a file, or replace one whole, in the open project: how to write a new screen\'s .tsx and .module.css, one call each. The canvas re-renders it as it lands. Creating needs path and content. Replacing an EXISTING file needs expectedHash from studio_read_file; without it, or if the file changed since, it refuses stale-source (for a small change use studio_edit_file). Refusals: needs-user for files that run on the user\'s machine (*.config.*, package.json, .env*, .npmrc, git hooks, .vscode/, CI) or CLAUDE.md, so show the user the change and ask them to make it; protected-path for .studio/, .claude/, .git/, node_modules/, build output and key material; path-outside-project. Returns { path, created, bytes, hash }; hash is the new version, for the next edit. Requires studio.write.',

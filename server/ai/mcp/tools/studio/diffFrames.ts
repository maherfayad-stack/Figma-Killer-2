...
export const diffFramesTool: AiTool = {
  ...
  sideEffects: 'none',
  description:
    'Server-side pixel and region diff of baseline (base64 PNG you hold) against reference (base64) or referenceId (a registered reference, read here). Returns a similarity score, a diff PNG and the top differing rectangles with diffPercent and, given nodeRects, the node ids inside each. Inside Studio use studio_compare instead: it captures and diffs in one call. With reference, both sizes must match or it returns ok:false. With referenceId, a size mismatch is resampled (dimensionReconciliation.method \'resampled\', a weaker claim); a taller capture of the same width is diffed over the top comparedHeight band only (\'cropped-to-reference\', unmeasuredHeight names the rest); any other aspect divergence, including a shorter capture, is refused.',

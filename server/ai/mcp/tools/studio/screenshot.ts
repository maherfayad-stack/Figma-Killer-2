...
export const studioScreenshotTool: AiTool = {
  ...
  description:
    'See what a screen actually looks like: renders each requested screen and returns it as a PNG image, placing a board frame for any page file that has none. This is how you check your own work: write the files, then look. Renders headless on the server from what is on disk (an open tab only as a fallback; capturedVia says which). Name screens as you named the files ("Checkout"), or omit pages for every screen. widths (e.g. [375, 768, 1280]) captures each screen at those widths without resizing the board; each frame then carries requestedWidth. Each result has width/height, its image index, nodeRects (node id → frame-local rect) and, for a .tsx/.jsx screen, canonical: { isCanonical, violations, advisories }, the canonical-JSX self-check (non-literal className, spread props, CSS-in-JS, unresolvable maps). It does not catch hard-coded colours or widths.',

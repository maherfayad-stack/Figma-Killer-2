...
A user opens a React project (hand-written, uploaded, or pulled from GitHub) as an infinite board of live, editable frames. Every edit is written back into the actual `.tsx` files as a precise AST change. There is no export step and no code generation: the repository *is* the document. Projects live in `studio-workspace/<project>/`. The editor is at **`/admin/site`** — `src/admin/router.tsx` renders it there unconditionally; there is no mode flag and no `?studio` param. Which project is open comes from `src/admin/pages/site/studio/studioWorkspaceDir.ts` (localStorage-sticky, set by the Overview launcher; with no selection the server falls back to the first project on disk).

Two invariants explain most of the codebase:

1. **Parse, never execute.** Everything on the canvas was read out of the AST by a bounded evaluator with explicit tiers — no component is rendered server-side, no hook is called. *(The per-project trust tier has shipped, and it is narrow. `.studio/meta.json`'s `trust` field has three values — `static` (Tier 0, the never-auto-promoted default), `render-packages` (Tier 1), `run-project` (Tier 2) — read and written by `server/handlers/studio/trustTier.ts`. At Tier 0 nothing of the user's ever runs. Promoting to Tier 1 buys exactly two things: the workspace's own style toolchain compiles in a capped subprocess (`styleCompileTier1.ts`), and its package components are bundled and rendered in the canvas (`componentBundle.ts`). **The parse itself never executes anything at any tier.** Promotion to Tier 1 is always an explicit user click. Promotion to **Tier 2 has one narrow exception, called by the owner on 2026-09-17** (`STUDIO-FIGMA-FEEL-PLAN.md` §6 decision 2): a project detected as **Vite with a lockfile** is promoted to `run-project` automatically on first open — once, ever — with a one-line notice in the board chrome and an **Undo** that writes `trust` back to `static` (`LiveAutoPromoteNotice.tsx`). The origin is recorded on disk (`trustAutoPromoted`/`trustAutoPromotedAt`), the latch means a project whose owner clicked Undo is never auto-promoted again, and every condition is re-checked server-side in `trustTier.ts` — a client that asks for it is requesting a gate, not passing one. A non-Vite project is never touched and the canvas says "Live needs Vite" instead. **Read that promotion for what it is: Tier 2 for a Vite project is a PRODUCT DEFAULT, not a consent boundary** (`sec-12`) — with it, both of `studio_render_reference`'s gates are satisfied by default for every project Studio can run, and no human answers a question. Anything that needs a human to have agreed must ask at the point of use, and the single-operator posture is what makes the default acceptable at all. Tier 2 (`run-project`) has its own dedicated gates, distinct from Tier 1's `trust !== 'static'` check: a preview deploy (`deploy.ts`) and the dev-server process manager (`server/handlers/studio/devServer.ts`, Track L's `live-01`) both refuse with 409 unless `trust === 'run-project'` exactly, via the shared `requireTrustTier` helper in `server/handlers/studio/trustGate.ts`.)*
...
This repo began as a fork of a self-hosted CMS. That subsystem — Postgres/SQLite, `data_tables`/`data_rows`, the publisher, plugins — **is still here and is load-bearing**: Studio's editor store, page tree, module engine, canvas, admin shell, and auth are all built on it.

The standalone **Content / Data / Media workspace routes were deleted** (PR #18): `src/admin/router.tsx` now serves only `/admin/dashboard` (the Studio launcher, "Overview"), `/admin/site` (the editor), `/admin/account`, and `/admin/plugins/:pluginId/:pageId`; Plugins, Users, and AI management moved into the Settings modal. Any other `/admin/*` URL redirects to the dashboard. The `data_tables`/`data_rows` model underneath still powers loops, data pickers, and publish — there is just no workspace UI for it anymore.
...
- **Content model (the CMS half only — Studio's content is the `.tsx` on disk):** DB-backed content lives in `data_tables` + `data_rows`. The four system tables (`posts`, `pages`, `components`, `layouts`) are seeded by the migrations and locked from rename/delete (`system = true`), gated by `data-tables-system-flag.test.ts`. There are no separate `pages` or `page_versions` tables.
...
- **MCP server:** Studio exposes its tools — the CMS tool engine plus the `studio_*` family in `server/ai/mcp/tools/studio/` (project reads, edits, typecheck, git, and the visual-audit loop) — to external MCP clients (Claude Code, Codex, remote agents) at `/_studio/mcp` (`MCP_ENDPOINT_PATH`), authenticated by per-connector bearer tokens. Thin adapter over the existing tool engine: headless reads and explicit `site_publish` run in-process; browser tools route by scope to the connector owner's **open Site workspace** through the `(userId, scope)` live bridge (`server/ai/mcp/editorBridge.ts` + `useMcpWorkspaceBridge`) — `EditorBridgeScope` is `` `site:${projectKey}` ``, one bridge per open project, since the Content workspace no longer exists and a user may have two projects open at once. The live workspace is the single source of truth for edits (no headless DB-mutating page-tree tool, which would desync), and writes remain drafts until the explicitly capability-gated publish call. Feature doc: [`docs/features/mcp-connectors.md`](docs/features/mcp-connectors.md).
...
- **Publishing:** Three-layer pipeline. **Layer A** bakes fully-static pages to `uploads/published/current/<route>.html` at publish time via a two-slot symlink swap (`server/publish/staticArtefact.ts`). **Layer B** is an in-memory LRU keyed by `(urlPath, queryString, publishVersion)` for dynamic routes (`server/publish/renderCache.ts`); `bumpPublishVersion()` evicts wholesale on every publish. **Layer C** emits `<studio-hole>` placeholders for nodes auto-detected as request-dependent; a ~1 KB `IntersectionObserver` runtime (`HOLE_RUNTIME_JS`, served at `/_studio/hole-runtime.js`) lazy-fetches each fragment from `/_studio/hole/<nodeId>`. Auto-detection lives in `src/core/publisher/dynamicDetection.ts` — one walker, four rules. Single entry: `server/publish/publicRouter.ts:renderPublicResolution`. Full design: [`docs/features/publisher.md`](docs/features/publisher.md).
...
server/handlers/studio/   Studio's server half (parse, writeback, git, trust tiers, capture)
src/admin/      Admin app (React) — shell, dashboard launcher, plugin host UI
src/admin/pages/site/   Visual editor (canvas, panels, toolbar, editor store)
src/core/       Engine: page parser, AST + CSS codemods, page tree, publisher, plugin SDK
src/modules/    Block modules — `base/` (container, text, image, button, …),
                `studio/` (slot), `alm/` (design-system pack)
src/ui/         Shared UI primitives (Button, Input, Tree, icons, cn)
src/styles/     Global tokens (globals.css)
scripts/        Bun task scripts (dev, vite, benches, icons:sync, bootstrap:sync)
...
- **No deprecated token families.** The `--editor-*`, `--rail-tint-*`, `--tag-pill-tint-*`, `--panel-bg/border/shadow*`, `--input-bg/border/shadow*`, `--tooltip-bg/fg/border/shadow`, `--spotlight-*` and `--code-bg` names are **banned** — admin chrome uses the global token vocabulary directly. Gated by `css-token-vocabulary.test.ts` (which also scans `docs/design.md`, `docs/reference/design-tokens.md`, `docs/reference/ui-primitives.md`).
- **Admin font sizes use the fluid `--text-*` scale; admin spacing uses the fluid `--space-*` scale.** Gated by `admin-typography-token-policy.test.ts` and `admin-spacing-token-policy.test.ts`.
- **Two-layer color model.** Surfaces, borders, and default text are achromatic (`--bg-body`, `--bg-surface`, `--bg-surface-2..5`, `--text-bright/--text/--text-muted/--text-subtle`, `--border`, `--overlay-*`, `--scrim-*`). Color is used as **identity** (the numbered accents `--accent-1..10`, each with a 10% surface tint `--accent-N-10`), as **state** (`--danger*`, `--warning*`, `--success*`, `--info-text`), or as **canvas affordance** (`--canvas-selection-ring`, `--canvas-hover-ring`). Never decorative.
- **Card surface pattern.** Tile cards (dashboard widgets and equivalents) are borderless: `--bg-surface-2` on a darker `--bg-surface` parent with a 1px grid gap and `--card-radius`. Hover lifts the surface tone to `--bg-surface-3` — never recolor a border. Canonical implementation: `src/ui/components/Widget/Widget.module.css`.
- **Border radius scale.** `--radius-sm` (3px) for tight chips/badges. `--radius` (6px) for default editor controls and buttons. `--panel-radius` (12px) for floating overlay panels. `--card-radius` (16px) for tile cards. `--input-radius` (1em) for pill-shaped inputs. `--tooltip-radius` (6px). The properties panel has its own inspector geometry tokens (`--inspector-*`). Don't introduce ad-hoc radius values.
...
- **No `!important`** in component CSS modules. Exactly one legitimate use remains in the tree: the `prefers-reduced-motion` block in `globals.css`. Out-specify with a descendant selector instead — see the comments in `CommentsPanel.module.css` and `StickyNoteView.module.css` for the pattern.
...
The 13 named tree-mutation store actions (`insertNode`, `deleteNode`, `updateNodeProps`, `setBreakpointOverride`, `clearBreakpointOverride`, `renameNode`, `toggleNodeLocked`, `toggleNodeHidden`, `moveNode`, `duplicateNode`, `wrapNode`, `groupNodes`, `ungroupNode`) are one-liners that call `mutateActiveTree`. They MUST NOT contain their own `kind === 'visualComponent'` routing branch — gated by `no-vc-mode-branches-in-mutations.test.ts`, whose `ACTION_PATHS` map is what you extend when a new action lives in a new module (`groupNodes`/`ungroupNode` are in `site/groupActions.ts`, not `nodeActions.ts`).

Plugins reach the same 13 mutations through `applyTreeOperation(tree, op)` — exported from `@core/page-tree`, dispatched on `op.kind`. It is the same engine the editor exercises via `mutateActiveTree`; the plugin RPC `cms.content.tree.mutate` runs each operation through it so plugin code rides the editor's gates instead of bypassing them.
...
- `@core/fonts`
- `@core/design-tokens`
- `@core/studio-comments` — review threads. `agentGate.ts`'s `isAgentActionable` is the single predicate deciding whether the agent may act on a thread; a deep import is how a second, looser copy of that decision gets made
...
- **One reason per module.** Files in `server/{handlers/studio,repositories,handlers/cms,auth,plugins,publish}/*` and `src/core/*` are organized by responsibility — keep them that way.
